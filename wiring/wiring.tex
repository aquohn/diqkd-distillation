...
\item[]}{\end{list}}
  ...
  \begin{align}
    ...
    \begin{align*}
      ...
      \end{gather}
      where \(\cvec{a}^c = {(a_1, a_2, \ldots, a_c)}^{\rm T}\) is the vector of all the outputs from Alice's boxes that are wired together, with analogous definitions for \(\cvec{b}^c\), \(\cvec{x}^c\) and \(\cvec{y}^c\). In this context, we refer to \(\chi\) as the \emph{wiring function}. The set of no-retrocausation conditions is more complex, since the later boxes can depend on the output of the earlier boxes. If Alice and Bob interact with their boxes in increasing order of the index \(j\), we have:
      \begin{equation}
        \begin{aligned}[c]
      ...
      \end{equation}
      If we focus only on \(j = c\), we can see these distributions as elements of \(\Ls\), with outputs \((a, \cvec{x}^c)\) and \((b, \cvec{y}^c)\), and inputs \((\cvec{a}^{c-1}, x)\) and \((\cvec{b}^{c-1}, y)\). However, unlike the simple LOSR case for a single box, the space of possible distributions \(\chi\) may be a strict subset of \(\Ls\), due to the additional no-retrocausation conditions for \(j < c\). We next turn our attention to characterising this space.

      \subsection{Generating the Wiring Polytope}

      To determine the polytope of possible wirings, we can use the same approach as is typically used for \(\Ls\), where deterministic operations are taken as the vertices of the polytope, and the facet-defining inequalities derived from there. However, we must also prove that the analogue of Fine's theorem holds for this more restricted polytope.
      \begin{theorem}[Deterministic Decomposition]
        Any wiring function can be written as a convex combination of deterministic wiring functions. For a given setting \((i_A, o_A; i_B, o_B)\) and number of wirings \(c\), the valid deterministic wirings form a finite, discrete set. Formally, denote the corresponding index set by \(\Lambda_D\). Then any wiring function can be written as
        \begin{equation}
          \chi_A(ab\cvec{x}^c\cvec{y}^c|\cvec{a}^c\cvec{b}^cxy) = \sum_{\lambda\in\Lambda_D} \chi(a\cvec{x}^c|\cvec{a}^cx \lambda) \chi_B(b\cvec{y}^c|\cvec{b}^cy \lambda) p_{\Lambda}(\lambda),
        \end{equation}
        where \(\chi_A(a\cvec{x}^c|\cvec{a}^cx \lambda)\) and \(\chi_B(b\cvec{y}^c|\cvec{b}^cy \lambda)\) only take values in \(\{0,1\}\), for some probability distribution \(p_{\Lambda}\) on \(\Lambda_D\).
      \end{theorem}
      \begin{proof}
        We begin using Fine's theorem to obtain a decomposition in terms of local deterministic operations. Let the indices for local deterministic operations that do not respect the no-retrocausation conditions form a set \(\Lambda_R\). Then we have that
        \begin{equation}
          \chi(ab\cvec{x}^c\cvec{y}^c|\cvec{a}^c\cvec{b}^cxy) = \sum_{\lambda\in\Lambda_D\cup\Lambda_R} \chi(a\cvec{x}^c|\cvec{a}^cx \lambda) \chi_B(b\cvec{y}^c|\cvec{b}^cy \lambda) p_{\Lambda}(\lambda).
        \end{equation}
        We now have to show that there always exists a distribution \(\forall \lambda\in\Lambda_R,\, p_{\Lambda}(\lambda) = 0\) whenever the no-retrocausation conditions are satisfied.
      \end{proof}

      % TODO need to re-prove Fine's theorem?
      % TODO relabel with lambda?

      For ease of implementation, we use a different, more operational description for deterministic wirings. We describe Alice's inputs \(\cvec{x}^c\) to her boxes as being computed by a sequence of \(c\) functions with range \(\dintv{1}{i_A}\):
      ...
      The corresponding \(\chi_A\) is then
      \begin{equation}
        \chi_A(a\cvec{x}^c|\cvec{a}^cx\lambda) = \indic{x=x_1} \indic{a = W_A(\cvec{x}^{c}, \cvec{a}^{c})} \prod_{j=2}^c \indic{x_j = W^{(j)}_A(\cvec{x}^{j-1}, \cvec{a}^{j-1})}
      \end{equation}
      ...
      We refer to the discrete set of deterministic wirings that obey these relations as \(\sW_D\). Since we can apply pairs of wirings to behaviours, so the set of wirings that can be applied is actually the convex hull \(\sW\) of \(\sW_D \times \sW_D\). We also define \(\sWB(p)\) as the set of behaviours generated via applying \(\sW\) to a given behaviour \(p\) using \Cref{eqn:wiringfndef}.
      ...
      Nonetheless, it does not seem feasible to use the unmodified coupler polytope beyond the simplest case. The authors identified the polytope of wirings by applying normalisation constraints \(0 \leq P'(\cvec{a\breve{b}}b'|\cvec{x\breve{y}}) \leq 1\), setting the values of \(\chi(b',\cvec{by})\) as variables and the values of \(P(\cvec{a\breve{b}b}|\cvec{x\breve{y}y})\) to those corresponding to the extremal points of the relevant version of \(\NSs\). In particular, they considered the case where Alice has no boxes and Bob has two binary-input, binary-output boxes, where \(P(\cvec{a\breve{b}b}|\cvec{x\breve{y}y})\) is then a behaviour in the familiar CHSH setting, except it describes the arbitrary no-signalling relationship between Bob's two boxes, rather than Alice and Bob's boxes. This approach is difficult to generalise to \(c > 2\). For example, for the simplest tripartite scenario of \((2,2; 2,2; 2,2)\), \(\NSs\) has 53856 extremal points~\cite{Tripartite}, and since the polytope of wirings is defined by two inequalities per extremal point, this would yield a polytope defined by 107712 inequalities.

      TODO classify the extremal points for the QKD setting

      Instead, we apply the methodology

      TODO couplers with \(o_B \times i_B\) outcomes, and input distributions are iid products of the arbitrary single-box distribution characterised by \(o_B \times i_B\) extremal points
    ...
    The set of \emph{bounded operators} \(\mathcal{B}(\Hs)\) on the Hilbert space \(\Hs\) is the subset of \(\mathcal{L}(\Hs)\), the set of all linear operators on \(\Hs\), such that the \emph{operator norm} of \(X \in \mathcal{L}(\Hs)\)
      ...
      A possible approach is to graft the wiring function \(\chi(ab\cvec{x}^c\cvec{y}^c|\cvec{a}^c\cvec{b}^cxy)\) directly onto this semidefinite program, or more specifically \(\chi_A(a\cvec{x}^c|\cvec{a}^cx \lambda)\) and \(\chi_B(b\cvec{y}^c|\cvec{b}^cy \lambda)\), as defined in \Cref{eqn:wiringfndef}. Since \(\chi\) takes a discrete set of arguments, these can be added as optimisation variables, and constrained with the no-retrocausation conditions in \Cref{eqn:wiringnoretro}, and the appropriate normalisation:
      \begin{equation}
        \begin{aligned}[c]
