...
If additionally \(\rho \leq \lambda\sigma\) for some \(\lambda \in \R_+\), then the \(i=m\) optimisation can be bounded analytically. We additionally collect the terms that are not multiplied with the optimisation variable \(Z\), removing the \(\Tr{\rho}\) terms from the optimisation and yielding the constant
    ...
    \begin{equation}
      ...
      \tikzmath{\w1 = 5em; \h1 = 1.5em;
      \olap1 = 0.3*\w1; \gap1 = 0.5*\w1;}
      \node[nclassbox,
        minimum width = \w{1},
        minimum height = \h{1},
        ] (state1) at (0, 0) {\(\rho\)};
      \draw[nclassinfl, mid arrow] (state1.north east) ++(-\olap1, 0) 
        -- ++(0, \h1) -- ++(\gap1, \h1) coordinate (temp)
        +(-\olap1 + 0.5*\w1, 1.5*\h1)
        node[rectangle,
        ...
        (temp) -- (temp |- N1.south);
      \draw[nclassinfl, mid arrow] (state1.north west) ++(\olap1, 0) 
        -- ++(0, \h1) -- ++(-\gap1, \h1) coordinate (temp)
        +(+\olap1 - 0.5*\w1, 1.5*\h1)
        node[rectangle,
        draw, double, rounded corners,
      ...
      \draw[mid arrow, classinfl] (M1.south west) ++(\olap1, 0) coordinate (M1 in)
        (M1 in |- {{(0, -2.5*\h1)}}) -- ++(0, \h1) node[right] {\(x_0\)} -- ++(0, \h1) coordinate (temp);
      \draw[classinfl] (temp) -- (M1 in);
      \draw[mid arrow, classinfl] (M1 in |- M1.north) -- ++(0, \h1) node[right] {\(a_0\)} -- ++(0, \h1);

      \draw[mid arrow, classinfl] (N1.south east) ++(-\olap1, 0) coordinate (N1 in)
        (N1 in |- {{(0, -2.5*\h1)}}) -- ++(0, \h1) node[left] {\(y_0\)} -- ++(0, \h1) coordinate (temp);
      \draw[classinfl] (temp) -- (N1 in);
      \draw[mid arrow, classinfl] (N1 in |- N1.north) -- ++(0, \h1) node[left] {\(b_0\)} -- ++(0, \h1);

    \end{tikzpicture}

      \begin{tikzpicture}[x=1pt,y=1pt]
      \tikzmath{\w1 = 5em; \h1 = 1.5em;
      \w2 = 7em; \h2 = 1.5em; \inflh = 0.2*\h2;
      \olap2 = 0.1*\w2; \gap2 = 0.1*\w2; 
      \olap1 = 0.3*\w1; \gap1 = 0.5*\w1;}
      ...
      \node[classbox,
        minimum width = \w{2},
        minimum height = \h{2},
        ] (chiA in) at (-\w2-\gap2, 2.5*\h2) {\(\chi_A^{(1)}\)};

      \node[classbox,
        minimum width = \w{2},
        minimum height = \h{2},
        ] (chiB in) at (\w2+\gap2, 2.5*\h2) {\(\chi_B^{(1)}\)};

      \draw[classinfl, mid arrow] (SR.north) ++(\olap2, 0) -- ++(0, \inflh) coordinate (temp1) 
        (chiB in.south west) ++(\olap2, 0) -- ++(0, -\inflh) coordinate (temp2)
        (temp1) -- (temp2);
