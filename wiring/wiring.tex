...
\item[]}{\end{list}}
  ...
  \begin{align}
    ...
    \begin{align*}
      ...
      A step towards this simplification would be another change of perspective. A deterministic marginal wiring map can be considered as a local transformation on a single-player behaviour. We therefore now consider the case of a single player, with input \(z_1 \in \dintv{1}{i_1}\) and output \(c_1 \in \dintv{1}{o_1}\). Each wiring map is now viewed as a block matrix \(\matrp{W}{^{(j)}}\), acting on the single-player behaviour represented as a vector \(\cvec{p}^{(j)}\).

      We vectorise the behaviour in column-major order, that is
      \begin{equation}
        p_k = p(c_1|z_1) \Leftrightarrow k = (c_1-1) \times o_1 + z_1.
      \end{equation}
