...
\item[]}{\end{list}}
  ...
  \begin{align}
    ...
    \begin{align*}
      ...
      \end{equation}
      where the breve indicates the inputs and outputs corresponding to boxes that were not coupled together, and \(b'\) is the coupler output. Comparing this to \Cref{eqn:wiringprobdef}, we see that, correctly interpreted, the coupler is simply a wiring performed only on Bob's boxes. \(b'\) is then the overall output \(b\) in our notation, while \(y\) in our notation is equal to the first element of \(\cvec{y}\), the vector of inputs to the boxes that are coupled/wired together.

      However, the analysis also differs in important ways. The distribution \(p(\cvec{b}^c|\cvec{y}^c)\) is allowed to be an arbitrary behaviour in \(\NSs\) in their analysis, not necessarily and i.i.d.\ joint distribution, with the only constraint being the normalisation of \(p'(\cvec{a\breve{b}}b'|\cvec{x\breve{y}})\). Therefore, \(p(\cvec{b}^c|\cvec{y}^c)\) can be any behaviour in the \(c\)-partite \(\NSs\), which is markedly more complicated than the bipartite scenarios we have assumed throughout this analysis.

      TODO do we proceed with this? Why or why not?
