...
\item[]}{\end{list}}
  ...
  \begin{align}
    ...
    \begin{align*}
      ...
      Therefore, for our purposes, we can focus on the set of deterministic wirings \(\sW\). This means that the set of free actions we are interested in is no longer a continuous convex set, but a \emph{discrete} set of transformations, characterised by conditional distributions \(\chi\) satisfying
      ...
      \end{equation}
      where \(\cvec{a}\) is the vector of all the outputs from Alice's boxes that are wired together, analogously for \(\cvec{b}\), \(\cvec{x}\) and \(\cvec{y}\), and the condition that \(\chi \in \{0,1\}\) enforces that the wirings are deterministic. In fact, we can restrict the possible wirings even further with causality constraints.
