...
\item[]}{\end{list}}
  ...
  \end{equation}
  Being the rate for an explicit, achievable protocol, this rate is a lower bound for the total secrecy extractable. It is tight amongst bounds that depend only on \(Q\) and \(S\).
  ...
  \subsection{AND Gates}
    ...
    \end{equation}
    where juxtaposition is the Boolean AND, addition is the Boolean OR, and the overbar is the Boolean NOT.\ This cannot be expressed as one of the wirings from~\cite{ShortEntangleSwap}.
