      ...
      & -\eta_B(1-\eta_A)\angleb{\tilde{B}_y} + (1-\eta_A\eta_B) )
            ...
            We verify that conditional distributions obeying the no-retrocausation conditions can be decomposed as a sequence of maps using Bayes' rule:
              ...
              \begin{proof}
                ...
                Select a particular \(k\in\dintv{1}{c+1}\), \(\hat{s}^{(k)}\) and \(\hat{t}^{(k)}\) such that \(\chi_{A|\lambda}^{(k)}(\hat{s}^{(k)}|\hat{t}^{(k)}) = 0 < q < 1\). If no such values exist, the wiring is already deterministic, and we are done.
                Now, consider wirings \(\chi_{A|\lambda,0}\) and \(\chi_{A|\lambda,1}\), such that
                \begin{align*}
                  \chi^{(k)}_{A|\lambda,0}(s^{(k)}|\hat{t}^{(k)}) &= \begin{cases}
                    0 & \text{ if } s^{(k)} = \hat{s}^{(k)} \\
                    \frac{1}{1-q} \chi_{A|\lambda}^{(k)}(s^{(k)}|\hat{t}^{(k)}) & \text{ otherwise,} \\
                  \end{cases} \\
                      \chi^{(j)}_{A|\lambda,0}(s^{(j)}|\hat{t}^{(j)}) &= \chi_{A|\lambda}^{(j)}(s^{(j)}|\hat{t}^{(j)}) \text{ otherwise,} \\
                    \chi^{(k)}_{A|\lambda,1}(s^{(k)}|\hat{t}^{(k)}) &= \begin{cases}
                      1 & \text{ if } s^{(k)} = \hat{s}^{(k)} \\
                      0 & \text{ otherwise,} \\
                    \end{cases} \\
                      \chi^{(j)}_{A|\lambda,1}(s^{(j)}|\hat{t}^{(j)}) &= \chi_{A|\lambda}^{(j)}(s^{(j)}|\hat{t}^{(j)}) \text{ otherwise.} \\
                    \end{align*}
                    It is easily verified that both \(\chi_{A|\lambda,0}^{(k)}(s|t)\) and \(\chi_{A|\lambda,1}^{(k)}(s|t)\) are valid maps, and hence \(\chi_{A|\lambda,0}\) and \(\chi_{A|\lambda,1}\) are valid marginal wiring distributions. Then, we have \(\chi_{A|\lambda} = (1-q)\chi_{A|\lambda,0} + q\chi_{A|\lambda,1}\). We can repeat this process recursively on \(\chi_{A|\lambda,0}\) and \(\chi_{A|\lambda,1}\) until \(\chi_{A|\lambda}\) is decomposed completely into a weighted sum of deterministic wiring distributions
