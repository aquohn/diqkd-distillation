...
E^{cc}_{sq,dev}(p,\prin) &= \inf_{\substack{\rho, \sM : \\ p = \behav{\rho}{ \sM}}} E^{cc}_{sq}(\rho, \sM, \prin).
    ...
    \begin{equation}
      ...
      & -\eta_B(1-\eta_A)\angleb{\tilde{B}_y} + (1-\eta_A\eta_B) )
              ...
              \begin{figure}
                \centering
                \includegraphics[width=0.9\linewidth]{exptplt.pdf}
    ...
    \Cref{thm:outdep}, and the discussion of correlated inputs from private shared randomness in this section and in \Cref{sec:diqkd_behavsec}, suggests a connection between the advantage of wirings in DIQKD and the \emph{measurement dependent scenario}, first studied in~\cite{RelaxedBell}, where local statistics are described by~\cite[Eq. 2]{MDLBeyondIID}
              ...
              \end{table}
                  ...
                  TODO we have 144 vertices and 256 inequalities for coupling a \((2,2)\) and a \((2,3)\) box together

                  We have reproduced the authors' computations in the \verb`ns_couplers_hrep` function in \verb`polytope.jl`, using the \verb`Polyhedra.jl` interface~\cite{Polyhedra} and the \verb`lrs` library~\cite{LRS}. To simplify computations, the normalisation constraint is converted into an inequality by eliminating one value of \(b'\). A polytope with 82 vertices was recovered for the \((2,2;2,2)\) setting, agreeing with the authors' previous results, while a polytope with 144 vertices was derived by extending the above method to two boxes in the \((2,2;2,3)\) setting, disagreeing with the 176-vertex polytope identified in~\cite[Sec. 7.1]{JanLiThesis}. This latter polytope was derived by extending the functional definitions for the 5 classes of couplers identified in the \((2,2)\) case to \((2,3)\) and claiming them to be extremal, but there is no justification for such a claim, and as our computational results show, this claim is incorrect. The results in~\cite{JanLiThesis} on the claimed extremal classes of wirings (reviewed in \Cref{sec:preres}) are hence even more limited than they originally seemed.

                  It does not seem feasible to use this coupler polytope beyond the simplest case. The computation for the \((2,3)\) setting took 30 minutes on a standard laptop, running in single-threaded mode. If we wish to consider \(c=3\), for the simplest tripartite scenario of \((2,2; 2,2; 2,2)\), \(\NSs\) has 53856 extremal points~\cite{Tripartite}. Since the polytope of wirings is defined by two inequalities per extremal point, this would yield a polytope defined by 107712 inequalities, which is unlikely to be tractable.
...
Any[]
                  \end{lstlisting}
                  cannot be that we violate the inequalities, nor can it be that their wirings violate our conditions. Must be the same polytope with degeneracies removed in theirs. Unclear why they got this result since they only considered no-signalling boxes but our definitions apply to arbitrary, possibly signalling behaviours

                  TODO Complete positivity => causal?

                  TODO One intriguing extension would be to analyse these couplers to see if they do, in fact, correspond to physically realisable wirings. The results of~\cite{ShortEntangleSwap} imply that all binary-input couplers are simply wirings, but since we have extended the number of inputs to 3, this implication does not apply to our scenario. Although we have the raw data necessary, we were not able to do this in the course of this project.
                  \subsection{Key Rates of Wired Behaviours}\label{sec:locwir_kr}
                  ...
                  \end{align}
                  where \(\matr[_{\crv{A}|x}]{P}_{ab}\), \(\matr[_{\crv{B}|y}]{P}_{ab}\), \(\matr[_{\crv{AB}|xy}]{P}_{ab}\) and \(\matr[_{\crv{AB}|xy}]{\tilde{P}}\) have dimensions \(o_A \times o_A\), \(o_B \times o_B\), \(o_A \times o_B\), \(o_A \times o_B\) respectively, and we take the Moore-Penrose pseudoinverse, which, since \(\matr[_{\crv{A}|x}]{P}_{ab}\) and \(\matr[_{\crv{B}|y}]{P}_{ab}\) are diagonal, involves inverting their non-zero diagonal entries and leaving the zero entries in place.
      ...
      \end{equation}
                  ...
                  Markov chains
                    ...
                    \begin{align}
                      ...
                      \section{Exploiting the Structure of the No-Signalling Polytope}\label{sec:nspoly}
            ...
            \caption[Representation of behaviours that violate CHSH inequality]{Representation of behaviours that violate \(S \leq 2\), with CHSH facet in blue}%
                  ...
                  TODO Since wirings are linear operations, can we study the effect on a general wiring from the effect on the PR box?
                    ...
                    TODO rewrite below, we have proved this
