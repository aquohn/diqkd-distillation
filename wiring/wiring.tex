...
\item[]}{\end{list}}
  ...
  The behaviours we are most interested in are those in the \emph{quantum set} \(\Qs\), where \(\Ls \subsetneq \Qs \subsetneq \NSs\), which is the set of behaviours achievable according to quantum theory. This is important because, until quantum theory is superseded, the devices we engineer can only achieve behaviours within \(\Qs\). Notably, the quantum set is \emph{not} a polytope, with boundaries that are curved rather than defined by linear inequalities (see~\cite{QGeometry} for a recent review of the geometric structure of \(\Qs\)). This contributes very significantly to the difficulty of analysing DIQKD\@.
    ...
    \begin{equation}
       \Tr\left[{(U_A \otimes U_B)} \rho {(U_A \otimes U_B)}^{\dagger} \left(U_A M_{a|x} U_A^{\dagger} \otimes U_B N_{b|y} U_B^{\dagger} \right)\right] = \Tr\left[ \rho \left(M_{a|x} \otimes N_{b|y} \right)\right].
    \end{equation}
    ...
    \begin{defn}[LOPC Operations]
      ...
      \begin{enumerate}
        \item \(\Omega_A^{(k)}\) is a classical stochastic map (\Cref{def:classop}) from 
      \begin{equation}
    ...
    \item The overall map is described by
      \begin{gather}
        \crv{S}_A^{(k+1)}\crv{D}_A^{(k+1)}
        \crv{S}_B^{(k+1)}\crv{D}_B^{(k+1)}\crv{D}^{(k+1)}E^n =
        \crv{S}_A^{(k)}\crv{D}_A^{(k)}
        \crv{S}_B^{(k)}\crv{D}_B^{(k)}
        \crv{D}^{(k)}E^n \\
        \Omega^{(k)} = \Omega_U^{(k)}\left[\left(\Omega_A^{(k)} \otimes \Omega_B^{(k)} \otimes I_{\crv{D}^{(k)}} \otimes I_{E^n}\right)\left[\rho\right] \right].
      \end{gather}
    \end{defn}
    ...
    \begin{defn}[DIQKD Protocols]
      ...
      {\Omega[\rho]}_{\crv{K}_{A}^{\ell_n} \crv{K}_{B}^{\ell_n} E^n \crv{D}^n} \coloneqq \Tr_{\crv{D}^{(m_n)}_A \crv{D}^{(m_n)}_B} \left[ \Omega_n \left[ \rho_{\crv{A}^n\crv{X}^n \crv{B}^n\crv{Y}^n E^n} \right] \right],
      \end{gather}
...
E^{cc}_{sq,dev}(p,\prin) &= \inf_{\substack{\rho, \sM : \\ p = \behav{\rho}{ \sM}}} E^{cc}_{sq}(\rho, \sM, \prin).
    ...
    \begin{equation}
      ...
      & -\eta_B(1-\eta_A)\angleb{\tilde{B}_y} + (1-\eta_A\eta_B) )
              ...
              \begin{figure}
                \centering
                \includegraphics[width=0.7\linewidth]{exptplt.pdf}
              ...
              \begin{figure}
                % TODO redo this, label the boxes accordingly
                \centering
                \includegraphics[width=0.7\linewidth]{losr.pdf}
              ...
              \end{equation}
              where the independence of \(p_i\) from \(a_0\) follows by causality: the production of the input \(x_0\) cannot depend on the value of the output \(a_0\), since the input must be produced before the output is received. We emphasise that these are \emph{maps}, not conditional distributions calculated from some joint distribution: the conditional distribution \(p'(x_0|x a_0)\) derived from the joint distribution \(p'(a a_0 x_0|x)\), obtained after applying the operation, will in general depend on \(a_0\). The maps can generally be treated as conditional probabilities, with the operations of summing over events and manipulations using Bayes' rule working similarly, but they are defined independently of the behaviours they act on, and hence an expression like \(\chi(a_0)\) would not be meaningful without specifying the input distribution \(\prin(x)\) and the behaviour acted on \(p(a|x)\).

              LOSR operations\footnote{Note that there are some articles which use a slightly different definition of LOSR, such that \(\chi\) is restricted to a nonconvex subset of \(\Ls\). One such article is~\cite{DIQKD_Limits}, which defines quantum intrinsic nonlocality (\Cref{sec:diqkd_ubehav}) and proves it is monotone under LOSR using the different definition. Fortunately, the result still holds using the definition of LOSR we use. See~\cite[Sec A.2]{BellResourceTheory} for more details, and a justification for the choice of this definition.} are the extension of local operations to a multipartite setting, where each party applies a local operation, but the choice of local operations between different parties can be correlated via \emph{shared randomness}. Bipartite LOSR operations, for example, transform \(p(a_0 b_0|x_0 y_0) \mapsto p'(ab|xy)\) using a map \(\chi\):
              \begin{equation}
              ...
              Applying the same analysis to the local operations that constitute a wiring, a wiring of \(c\) boxes would need to generate inputs for the \(c\) boxes, and combine the \(c\) outputs into one output.
              ...
              \end{equation}
            These conditions will hopefully simplify the space of wirings, since without the conditions, we would have to handle \({(o_A i_A^c)}^{i_A^c o_A} {(o_B i_B^c)}^{i_B^c o_B}\) vertices in \(\Ls\) (\(\approx \num{5.12e18}\) for the QKD setting). We define the set of conditional probabilities of the appropriate cardinalities obeying the no-retrocausation conditions as \(\tilde{\sW}\). From this definition alone, the following result is immediate.
            \begin{lemma}[Structure of the Space of Wiring Distributions]
              \(\tilde{\sW}\) is convex, and the space of \(n\)-partite wiring distributions is the convex hull of the \(n\)-partite Cartesian product of \(\tilde{\sW}\).
            \end{lemma}
            \begin{proof}
              The no-retrocausation conditions are a set of linear equalities, and any linear combination of their solutions (which are precisely the elements of \(\tilde{\sW}\)) will satisfy them. Since convex combinations are a subset of linear combinations, any convex combination of elements in \(\tilde{\sW}\) will satisfy the conditions and be an element of \(\sW\).

            A joint wiring distribution for \(n\) parties is then the convex combination of \(n\)-tuples of marginal wirings, applied to behaviours as per \Cref{eqn:jwirdistdef}. We therefore abuse notation slightly by writing \(\tilde{\sW}^n\) to refer to the set of \(n\)-partite joint wiring distributions, even though this set is strictly richer than the simple Cartesian product of \(\tilde{\sW}\).\footnote{The spaces of marginal wiring distributions for different parties are not the same in general, making this even more of an abuse of notation, although one could argue that all of them are subspaces of the version of \(\tilde{\sW}\) with the maximal number of outputs and the maximal number of inputs, as per the argument in \Cref{sec:nl_ns}.} The convexity of \(\tilde{\sW}^n\) follows by definition.
            \end{proof}

            However, an important distinction to make is that the set of \emph{wiring distributions} is \emph{not} the set of \emph{wirings}: we have not proved that any conditional probability that obeys the locality and no-retrocausation conditions, that is, any \(n\)-partite wiring distribution, constructs a unique wiring, in the sense of an operation that maps one behaviour to another. As it turns out, there may be differences between wiring distributions that do not lead to any \emph{observable} difference: that is, we could have two wiring distributions \(\chi^{(1)}\) and \(\chi^{(2)}\) such that there are differences in their values, but \(\chi^{(1)}[p] = \chi^{(2)}[p]\) for all behaviours \(p\). We can therefore say that these wiring distributions describe the \emph{same} wiring, and, more generally, that the wiring distribution description contains some redundancy. Explicit examples illustrating such redundancy are provided in \Cref{sec:locwir_func}.

              On the other hand, each wiring distribution describes \emph{some} physically realisable transformation between behaviours, and for any wiring, its effect on all behaviours can be described using a wiring distribution. We formalise the relationship between wiring distributions and wirings as follows.
            \begin{theorem}[Sufficiency of Wiring Distributions]
              There exists a natural surjective map from \(\tilde{\sW}\) to the space of marginal wirings \(\sW\), and the space of \(n\)-partite wirings \(\sW^n\) is the convex hull of the \(n\)-times Cartesian product of \(\sW\).
            \end{theorem}
            \begin{proof}
                We can decompose a marginal wiring distribution using Bayes' rule as
                \begin{align}
                ...
                \end{gather}
                The extension of this proof to the last no-retrocausation condition is straightforward. This tells us that the set of marginal wiring distributions is isomorphic to the set of causal marginal distributions, where, we note, there are no constraints on the conditional distributions that are multiplied together, other than them being valid probability distributions and having the correct input and output cardinalities.

                Observe that this is simply the description of a sequence of probabilistic maps, generating the next input or output required based on existing data. Therefore, these represent the most general local operations possible, and are sufficient to characterise the space of marginal wirings \(\sW\), because expressing the effect of a marginal wiring in this way, which completely characterises the effect of the wiring, is sufficient to specify
            ...
            \end{proof}
              ...
              Just from this first characterisation of wirings, we are able to derive a fundamental limitation on achievable key rates via wiring.
      ...
      \begin{align*}
          ...
          p'(ab|xy) \rho_{E|abxy} &= \sum_{\lambda} P_{\Lambda}(\lambda) \Tr\left[ \rho_{ABE|\lambda} \left( M'_{a|x\lambda} \otimes N'_{b|y\lambda} \otimes I_{E} \right) \right] \\
    ...
    \Cref{thm:outdep}, and the discussion of correlated inputs from private shared randomness in this section and in \Cref{sec:diqkd_behavsec}, suggests a connection between the advantage of wirings in DIQKD and the \emph{measurement dependent scenario}, first studied in~\cite{RelaxedBell}, where local statistics are described by~\cite[Eq. 2]{MDLBeyondIID}
            ...
            \caption[Representation of behaviours that violate CHSH inequality]{Representation of behaviours that violate \(S \leq 2\), with CHSH facet in blue}%
                  ...
                  \end{equation}
                  In particular, this means that the maximal correlation of any number of iid copies of the pair \((\crv{A},\crv{B})\) is \(\varrho(\crv{A},\crv{B})\), even though the functions \(f\) and \(g\) can operate on all copies at once. Since wirings are functions of this form, this suggests that the maximal correlation constrains what wirings can achieve. While the proof of this fact is rather technical, this is indeed the case~\cite[Thm 8]{NLMonotones}, and extends even to wirings where the order in which the boxes are used is permuted, although the boxes are otherwise assumed to be iid, and the wirings are not permitted to use shared randomness. In contrast, our definition (\Cref{sec:locwir}) focuses on the wirings themselves, rather than their action on any specific behaviour. Shared randomness in the choice of the wiring is permitted, and permutations of the boxes used are encoded in the joint behaviour the wirings act on.
                  ...
                  TODO MC ribbon is fully characterised by MC, phi-entropies
