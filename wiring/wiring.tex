...
\item[]}{\end{list}}
  ...
  \pgfplotsset{%
    ...
    \begin{align}
      r\left(\qproto[\ell']{\Omega'}, {\{\rho_{\hat{A}\hat{B}E}^{n}\}} \right) &= r\left(\qproto[\bar{\ell}]{\bar{\Omega}}, {\{\rho_{\hat{A}\hat{B}E\bar{E}}^{n}\}} \right) + \lim_{n\to\infty} \frac{1}{n}H{(\crv{P}_{A}^n \crv{P}_{B}^n)}_{\tau^{n}} \\
                                                                               &\geq r\left(\qproto{\Omega}, {\{\rho_{\hat{A}\hat{B}E}^{n}\}} \right) - \lim_{n\to\infty} \frac{1}{n}(1+\indic{\bar{E}^n \text{ quantum}}){H(\bar{E}^n)}_{\rho^n} + \frac{1}{n}H{( \crv{P}_{A}^n \crv{P}_{B}^n )}_{\tau^{n}} \\
                                                                               &= r\left(\qproto{\Omega}, {\{\rho_{\hat{A}\hat{B}E}^{n}\}} \right),
    \end{align}
...
Any[]
          ...
          & P_{\Lambda}(\lambda) \chi_{A|\lambda}(a\cvec{x}|^c\cvec{a}^cx) \chi_{B|\lambda}(b\cvec{y}^c|\cvec{b}^cy) p^c(\cvec{a}^c\cvec{b}^c|\cvec{x}^c\cvec{y}^c) \rho_{E^c|\cvec{a}^c\cvec{x}^c \cvec{b}^c\cvec{y}^c\lambda}.
                        ...
                        \end{equation}\label{eqn:hatrdef}
