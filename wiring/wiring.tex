...
\item[]}{\end{list}}
  ...
  \begin{align}
    ...
    \begin{align*}
      ...
      For ease of implementation, we use a different, more operational description for deterministic wirings. Focusing on Alice's marginal wiring functions, her inputs \(\cvec{x}^c\) to her boxes as being computed by a sequence of \(c\) functions with range \(\dintv{1}{i_A}\):
      ...
      Using our classification for \(\sW_D\), simple combinatorics gives us \(i_A^{j} o_A^{j-1}\) possible inputs for \(W^{(j)}_{A,\lambda}\). However, since the wiring is deterministic, \(x_j\) is completely fixed by \(\cvec{x}^{j-1}\), \(\cvec{a}^{j-1}\) and \(x\). Therefore, when considering the function, the only free input that is provided to it as an argument is \(x\), and so there are only \(i_A o_A^{j-1}\) valid arguments. Formally,
      \begin{lemma}
        For any deterministic marginal wiring, for each wiring map \(W^{(j)}\),
        \[ W^{(j)}(\cvec{x}^{j-1}, \cvec{a}^{j-1}, x) = W^{(j)}(\cvec{x'}^{j-1}, \cvec{a}^{j-1}, x) \]
        for all input sequences \(\cvec{x'}^{j-1}\) and \(\cvec{x}^{j-1}\), where \(W^{(c+1)} = W\).
      \end{lemma}
      \begin{proof}
        If 
      \end{proof}
      ...
      Consider two boxes in the QKD setting. There are 16384 marginal wirings for Alice, and \(\approx \num{8.06e7}\) for Bob, for a total of \(\approx \num{1.32e12}\). Fixing the AND gating as discussed above gives us 128 wirings for Alice and 186624 for Bob, for a total of \(\approx \num{2.39e7}\). Both are much more manageable than the number of vertices of \(\Ls\) without causality constraints. Still, this enumeration of the possible wirings is naive, and does not attempt to use symmetry arguments to remove degeneracy or wirings that will not be helpful.

      A step towards this simplification would be another change of perspective, where each wiring map is now viewed as a block matrix, acting on the behaviour represented as a vector.
