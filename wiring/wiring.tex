...
\item[]}{\end{list}}
  ...
  \begin{align}
    ...
    \begin{align*}
      ...
      % TODO express as sequence of LD operations?
      ...
      Instead, we can adapt the methodology to our situation in order to derive the possible deterministic wirings for a single player. The different values of \(b'\) correspond to different values of the overall output \(b\), while the distributions \(P(\cvec{b}|\cvec{y})\) must be the product of \(c\) deterministic single-box wirings, of which there are \(o_B^{i_B}\).
