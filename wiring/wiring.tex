...
\item[]}{\end{list}}
  ...
  \pgfplotsset{%
    ...
    \newcommand{\proto}[2][\omega,\ell]{\mathfrak{#2}_{#1}}
    \newcommand{\qproto}[2][\omega,\ell]{\hat{\mathfrak{#2}}_{#1}}
    \newcommand{\prin}{p_{\mathrm{in}}}
    \newcommand{\sk}{\rm sk}
    \newcommand{\DW}{\rm DW}
    ...
    \begin{defn}[The Local Polytope]
      ...
      \end{equation}
      where \(P_{\Lambda}\) is an arbitrary probability mass function over an alphabet \(\{\lambda\}\), whose elements parametrise the family of distributions \(\{p_{\lambda}(a|x) = p(a|x\lambda), p_{\lambda}(b|y) = p(b|y\lambda)\}\).\footnote{\(\Lambda\) could be a continuous random variable as well, but we will assume that it is a discrete random variable for simplicity.}
    \end{defn}
    The choice to denote the behaviours as a family of distinct pmfs, as opposed to two pmfs conditioned on the same rv \(\Lambda\), will be made based on convenience of the exposition.
    ...
    \begin{defn}[DIQKD Protocols]
      ...
      \begin{equation}
        \left(\prod_{k=1}^{m_n} \Omega^{(k)}_{n} \right) \rho_{\crv{A}^n\crv{X}^n \crv{B}^n\crv{Y}^n E^n} {\left(\prod_{k=1}^{m_n} \Omega^{(k)}_{n} \right)}^{\dagger} = \omega_{\crv{K}_{A}^{\ell_n} \crv{K}_{B}^{\ell_n} E^n \crv{D}},
      \end{equation}
    where \(\crv{K}_{A}^{\ell}\) and \(\crv{K}_{B}^{\ell}\) are Alice and Bob's keys of length \(\ell\), respectively, while \(\crv{D}^n\) is all the public data that is communicated during the protocol.
    \end{defn}
    ...
    Alice and Bob share an \emph{\(\epsilon\)-secure key} iff their joint state with Eve and the publicly transmitted data \(\omega_{\crv{K}_{A}^{\ell} \crv{K}_{B}^{\ell} E^n \crv{D}^n}\) satisfies~\cite{SecurityQKD}
      \begin{equation}
        \Delta_{\Tr}(\omega_{\crv{K}_{A}^{\ell} \crv{K}_{B}^{\ell} E^n \crv{D}^n}, \tau_{\crv{K}_{A}^{\ell} \crv{K}_{B}^{\ell}} \otimes \rho_{E^n} \otimes \omega_{\crv{D}^n}) \leq \epsilon,
      \end{equation}
      ...
      \end{equation}
      and \(\omega_{\crv{D}^n}\) is the publicly exchanged data
      \begin{equation}
        \omega_{\crv{D}^n} = \Tr_{\crv{K}_{A}^{\ell} \crv{K}_{B}^{\ell} E^n}\left[ \omega_{\crv{K}_{A}^{\ell} \crv{K}_{B}^{\ell} E^n \crv{D}^n} \right],
      \end{equation} 
    ...
    \begin{equation}
      r(\proto{P}, \{\rho\}) = \lim_{n\to\infty} \frac{\ell_n}{n} \indic{
      \Delta_{\Tr}(\omega_{\crv{K}_{A}^{\ell_n} \crv{K}_{B}^{\ell_n} E^n \crv{D}^n}, \tau_{\crv{K}_{A}^{\ell_n} \crv{K}_{B}^{\ell_n}} \otimes \rho_{E^n} \otimes \omega_{\crv{D}^n}) = 0},
    \end{equation}
...
\end{enumerate}
    ...
    Let us now develop the relevant notions to describe what we can infer about the state. We say that a ccq state representing \(n\) rounds of Bell tests is \emph{compatible} with a behaviour \(p\) and input distribution \(\prin^n\) if its input distribution is equal to \(\prin^n\) and its joint conditional distribution \(p^n\) is averaged over \(n\) rounds, we recover the behaviour, and we denote the set of such states as \(\compatstates[^n]{p, \prin}\). TODO compatible states must have arisen from some kind of measurement, and can't just be some arbitrary system depending on their inputs

    \begin{defn}[States Compatible with Classical Data]
      A ccq state \(\rho\) representing \(n\) rounds of Bell tests, defined as per \Cref{def:ccq_belltest}, is an element of \(\compatstates[^n]{p, \prin}\) iff its \(p^n\) satisfies
      \begin{equation}
      ...
      \end{equation}
      and its input distribution is equal to \(\prin^n\).
    \end{defn}

    Due to statistical fluctuations from finite sample sizes, it may be advantageous to define a \emph{smoothed} version of the set of compatible states admitting states where \(p^n\) averaged over \(n\) rounds is \(\epsilon\)-close to the specified behaviour \(p\). For simplicity, however, we do not do so here, although we expect our results to carry through in the finite-key setting where such smoothing is necessary.
    \begin{question}
    ...
    \begin{equation}
      C_{\sk}(p) = \sup_{\proto{P}} \sup_{\{\prin^n\}} \inf_{\substack{\{\rho^{n}\} : \\ \rho^{n} \in \compatstates[^n]{p, \prin^n}}} r(\proto{P}, \{\rho^{n}\}) - \frac{L(\prin^n)}{n},
    \end{equation}
    where \(L(\prin^n)\) is the minimum number of bits in Alice and Bob's existing shared secrecy that must be expended in sampling from \(\prin^n\) with asymptotically vanishing error. The input distributions \(\prin^n\) are maximised over, since Alice and Bob are free to choose their inputs, but they cannot otherwise constrain Eve's choice of state, hence the inner minimisation on the state.
    \end{defn}

    This definition is novel in allowing correlated inputs and considering the secrecy cost of generating the input, which must be private from Eve, but is therefore more general and may lead to greater achievable rates. We will return to the issues and implications arising from the use of correlated inputs throughout our discussion, but we first remark that the cost \(L(\prin^n)\) is difficult in general to compute (except where \(\crv{X}^n\) and \(\crv{Y}^n\) are independent, in which case it is trivially zero). If \(\prin^n\) consists of iid copies of some distribution \(\prin\), then this reduces asymptotically to \(n\) times the \emph{Wyner common information} \(I^{\mathtt{Wyn}}\) between \(\crv{X}\) and \(\crv{Y}\), characterised by~\cite[Thm V.3]{CorrReview}
    \begin{equation}
      I^{\mathtt{Wyn}}(\crv{X}:\crv{Y}) = \min_{\crv{X} \markov \crv{U} \markov \crv{Y}} I(\crv{U}:\crv{XY}).
    \end{equation}
    In the general case, the entire sequences \(\cvec{x}^n\) and \(\cvec{y}^n\) are generated at once, so we require a one-shot characterisation. We do not require a precise characterisation, so we leave this issue aside presently, but we make a few remarks:
    \begin{itemize}
      \item The \emph{exact common information}~\cite{ExactCommonInfo}, which quantifies the amount of shared randomness required to exactly simulate a distribution, appears to be connected to this task, but relies on minimising over all possible shared rvs, while we only have shared uniform randomness, and would need to synthesise the optimal shared rv exactly as well.
      \item The requirement for vanishing single-shot error puts strong constraints on the possible form of the sequence \(\{\prin^n\}\). However, as the preceding discussion demonstrates, non-correlated (independent) variables and iid pairs of variables are possible realisations.
      \item There is a one-shot achievability result, the \emph{soft covering lemma}~\cite[Lem. IV.1]{DistrChanSynth}. In its simplest form, it bounds the total variational distance of a desired output distribution on \(\crv{V}^m\) from a distribution generated by sending random sequences from a subset of the alphabet of \(\crv{U}^{n}\) through \(m\) iid copies of a channel \(P_{\crv{V}|\crv{U}}\). If the sequences are sampled uniformly from a random subset of \(\{\cvec{v}^m\}\) of size \(> \exp_2(mI(\crv{U}:\crv{V}))\) (the \emph{codebook}), then the total variation distance vanishes exponentially in \(m\), and has an explicit bound for all \(m\). This can be converted to our problem setting by setting \(\crv{V} = \crv{X^n Y^n}\).  If \(\crv{S}\) is the rv representing the totality of their shared secrecy, since Alice and Bob have to generate their inputs locally, we must have \(P_{\crv{XY}|\crv{S}} = P_{\crv{X}|\crv{S}} P_{\crv{Y}|\crv{S}}\), if the desired \(P_{\crv{XY}}\) can indeed be thus generated.

      In fact, there exists an arbitrary random variable \(\crv{U}\) with bounded cardinality achieving the same~\cite[Sec. VI.A]{DistrChanSynth}

      TODO
    \item It may be worth utilising more recently developed tools to establish better one-shot converse~\cite{BLIneqStrongConv} and achievability~\cite{PoissonMatch} results in order to obtain tighter bounds on how much the freedom in choosing input distributions can help Alice and Bob. In particular, there does not seem to be a one-shot converse for Wyner common information problem.
    \item TODO Post-selection and recovery
    \end{itemize}
    ...
    \begin{align}
      \rho_{\crv{AXBY}E} = \sum_{abxy} \prin^n(x,y) p(ab|xy) \proj[\crv{AXBY}]{axby} \otimes \rho_{E|axby}.
    \end{align}
    ...
    \begin{defn}[The iid Secret Key Capacity]\label{def:seckeycapbehaviid}
      ...
      \begin{equation}
        C_{\sk}^{\otimes}(p) = \lim_{n\to\infty} \sup_{\proto{P}} \inf_{\rho \in \compatstates{p}} r(\proto{P}, \rho_{\crv{A}\crv{X} \crv{B}\crv{Y} E}^{\otimes{n}}).
      \end{equation}
    \end{defn}
  ...
  \end{defn}
    ...
    TODO remove this section, merge with above discussion about compat states
    ...
    \end{equation}
    The chosen measurement strategy and state are jointly referred to as the \emph{implementation} of a behaviour.
  ...
  \end{defn}
    ...
    \begin{equation}
      N^Q{(\crv{A}:\crv{B})}_p = \sup_{\prin(x,y)} \inf_{\rho_{\crv{ABXY}E}\in\compatstates{p}} I{(\crv{A}:\crv{B}|\crv{XY}E)}_{\rho}.
    \end{equation}
    ...
    \begin{align}
      ...
      E^{cc}_{sq,dev}(p,\prin) &= \inf_{\substack{\rho, \sM : \\ p = \cP(\rho, \sM)}} E^{cc}_{sq}(\rho, \sM, \prin).
    \end{align}
    ...
    TODO \(N^Q{(\crv{A}:\crv{B})}_p = 0 \Rightarrow p \in \Ls\), but \(N^Q\) is not a tight bound on the key rate, because we can have \(N^Q > 0\) for \(p(x,y)\) dependent, which would require the consumption of some shared secrecy.
    ...
    \begin{equation}
      ...
      & -\eta_B(1-\eta_A)\angleb{\tilde{B}_y} + (1-\eta_A\eta_B) )
              ...
              However, since our wirings combine multiple boxes, their effect cannot be expressed in this form. Nonetheless, we can straightforwardly generalise the equations describing the action of an LOSR operation, and the locality condition, to the case of \(c\) boxes with an arbitrary joint behaviour \(p^c\) wired together:
              \begin{gather}
              ...
              \end{gather}
              where \(\cvec{a}^c = {(a_1, a_2, \ldots, a_c)}^{\rm T}\) is the vector of all the outputs from Alice's boxes that are wired together, with analogous definitions for \(\cvec{b}^c\), \(\cvec{x}^c\) and \(\cvec{y}^c\), with \(x_1\) being the input of the first box Alice interacts with, \(a_1\) being its output, and so forth. We refer to the function \(\chi\) as the \emph{joint wiring distribution}, and \(\chi_{A|\lambda}\) and \(\chi_{B|\lambda}\) as the as the \emph{marginal wiring distributions}. We additionally write \(p' = \chi[p^c]\) to indicate that \(p'\) is the result of applying the wiring \(\chi\) to the joint behaviour \(p^c\). Now, these distributions are clearly elements of \(\Ls\), with outputs \((a, \cvec{x}^c)\) and \((b, \cvec{y}^c)\), and inputs \((\cvec{a}^{c-1}, x)\) and \((\cvec{b}^{c-1}, y)\). However, not every element of \(\Ls\) is a valid wiring, since the wirings are also constrained by the no-retrocausation conditions.
              ...
              These conditions will hopefully simplify the space of wirings, since without the conditions, we would have to handle \({(o_A i_A^c)}^{i_A^c o_A} {(o_B i_B^c)}^{i_B^c o_B}\) vertices in \(\Ls\) (\(\approx \num{5.12e18}\) for the QKD setting). We will shortly turn our attention to characterising this space.
                ...
                While the definitions and discussion thus far focus on the bipartite case, they generalise readily to the \(n\)-partite case. We will not discuss this case in detail, but will occasionally mention how this generalisation can be done. 

                Note that the definition of wirings in \Cref{eqn:jwirdistdef} is expressed in terms of \(p^c(\cvec{a}^c\cvec{b}^c|\cvec{x}^c\cvec{y}^c)\), the joint behaviour of the \(c\) boxes. This very general formulation accomodates situations where the individual boxes are not simply iid copies of each other, or where the order of the boxes is permuted. For independent boxes, we have most generally
                  \begin{equation}
              ...
              Just from this first characterisation of wirings, we are able to derive some limitations on what they can achieve.
              ...
              More concretely, consider any joint behaviour \(p^c\) generated by a quantum process on Alice and Bob's joint Hilbert space \(AB = A \otimes B\):
              \begin{equation}
                p^c(\cvec{a}^c\cvec{b}^c|\cvec{x}^c\cvec{y}^c) = \Tr\left[\rho_{AB} \left( M_{\cvec{a}^c|\cvec{x}^c} \otimes N_{\cvec{b}^c|\cvec{y}^c} \right) \right],
              \end{equation}
              and let \(C_{\sk}(p) = r\). Then, for any behaviour \(p'\) that can be obtained from applying a wiring without output dependency \(\chi\) to \(p^c\), we have
              \begin{gather}
                p'(ab|xy) = \sum_{\cvec{a}^c\cvec{b}^c\cvec{x}^c\cvec{y}^c} \sum_{\lambda} \chi_{A|\lambda}(a\cvec{x}^c|\cvec{a}^cx) \chi_{B|\lambda}(b\pi(\cvec{y}^c)|\pi(\cvec{b}^c)y) P_{\Lambda}(\lambda) p^c(\cvec{a}^c\cvec{b}^c|\cvec{x}^c\cvec{y}^c) \\
                = \sum_{\cvec{a}^c\cvec{b}^c\cvec{x}^c\cvec{y}^c} \sum_{\lambda} \chi_{A|\lambda}(a\cvec{x}^c|\cvec{a}^cx) \chi_{B|\lambda}(b\pi(\cvec{y}^c)|\pi(\cvec{b}^c)y) P_{\Lambda}(\lambda) \Tr\left[\rho_{AB} \left( M_{\cvec{a}^c|\cvec{x}^c} \otimes N_{\cvec{b}^c|\cvec{y}^c} \right) \right], \\
                = \sum_{\lambda} P_{\Lambda}(\lambda) \Tr\left[\rho_{AB} \underbrace{\left( \sum_{\cvec{a}^c\cvec{x}^c} \chi_{A|\lambda}(a\cvec{x}^c|\cvec{a}^cx) M_{\cvec{a}^c|\cvec{x}^c} \right)}_{M'_{a|x\lambda}} \otimes \underbrace{\left(\sum_{\cvec{b}^c\cvec{y}^c} \chi_{B|\lambda}(b\pi(\cvec{y}^c)|\pi(\cvec{b}^c)y) N_{\pi(\cvec{b}^c)|\pi(\cvec{y}^c)} \right)}_{N'_{b|y\lambda}} \right].
              \end{gather}
              ...
              \end{align}
               ...
               TODO explicitly construct expanded Hilbert space to show that Eve will have the shared randomness already

               Let \(C_{\sk}(p') = r'\), and let \(\rho_{\crv{A}^c\crv{X}^c \crv{B}^c\crv{Y}^c E}\) be the qqq state compatible with \(p\) with the best possible \(\prin^c\) for Alice and Bob, and the best possible \(E\) for Eve, while \(\rho'_{\crv{A}\crv{X} \crv{B}\crv{Y} E}\) is the ccq state with the best possible \(\prin\) for Alice and Bob and the implementation above (hence the same Hilbert space \(E\) used in both), with the Hilbert space expanded to remove the shared randomness \(\lambda\). Then we can write these states as
      \begin{align}
        ...
        \end{aligned} \\
          \rho'_{\crv{A}\crv{X} \crv{B}\crv{Y} E} &= \sum_{abxy} p'_{\rm in}(x,y) \proj[\crv{A}\crv{X} \crv{B}\crv{Y}]{axby} \otimes \Tr_{AB} \left[ \rho_{ABE} \left(M'_{a|x} \otimes N'_{b|y} \otimes I_E\right) \right] \\
                                                  &=
                                                  \sum_{abxy} p'_{\rm in}(x,y) \proj[\crv{A}\crv{X} \crv{B}\crv{Y}]{axby} \otimes \sum_{\cvec{a}^c\cvec{b}^c\cvec{x}^c\cvec{y}^c\lambda}
                                                  \begin{aligned}[c]
          & P_{\Lambda}(\lambda) \chi_{A|\lambda}(a\cvec{x}^c|\cvec{a}^cx) \chi_{B|\lambda}(b\pi(\cvec{y}^c)|\pi(\cvec{b}^c)y) \\
          &  p^c(\cvec{a}^c\cvec{b}^c|\cvec{x}^c\cvec{y}^c)
          \rho_{E|\cvec{a}^c\cvec{x}^c \cvec{b}^c\cvec{y}^c},
                                                  \end{aligned} \\
          &= 
          \sum_{abxy} p'_{\rm in}(x,y) \proj[\crv{A}\crv{X} \crv{B}\crv{Y}]{axby} \otimes \begin{aligned}[c]
          & P_{\Lambda}(\lambda) \chi(ab\cvec{x}^c\pi(\cvec{y}^c)|\cvec{a}^c\pi(\cvec{b}^c)xy) \\
          & \sum_{\cvec{a}^c\cvec{b}^c\cvec{x}^c\cvec{y}^c} p^c(\cvec{a}^c\cvec{b}^c|\cvec{x}^c\cvec{y}^c)
          \rho_{E|\cvec{a}^c\cvec{x}^c \cvec{b}^c\cvec{y}^c},
          \end{aligned} \\
      \end{align}
      By assumption, there must exist an LOPC protocol \(\mathfrak{P}'_{\omega}\) that distills \(\rho'\) at a rate of at least \(r'\), since this specific implementation could be suboptimal for Eve. However, by choosing 
      \begin{equation}
        \prin^c(\cvec{x}^c,\cvec{y}^c) = \sum_{xy} \chi(\cvec{x}^c\cvec{y}^c|xy)p'_{\rm in}(x,y) = \sum_{xy} \chi(\cvec{x}^c\cvec{y}^c|\cvec{a}^c\cvec{b}^cxy)p'_{\rm in}(x,y),
      \end{equation}
      and local operators
      \begin{align}
        V_A &= \sum_{ax\cvec{a}^c\cvec{x}^c} \ket{ax}\bra{\cvec{a}^c\cvec{x}^c} \\
        V_B &= \sum_{by\cvec{b}^c\cvec{y}^c} \ket{by}\bra{\cvec{b}^c\cvec{y}^c},
      \end{align}
      Alice and Bob can obtain
      \begin{gather}
        (V_A \otimes V_B \otimes I_E)\rho{(V_A \otimes V_B \otimes I_E)}^{\dagger} \\
        =   \sum_{\cvec{a}^c\cvec{b}^c \cvec{x}^c\cvec{y}^c}
          \prin^c(\cvec{x}^c,\cvec{y}^c) p^c(\cvec{a}^c\cvec{b}^c|\cvec{x}^c\cvec{y}^c)
          \sum_{abxy} \proj[\crv{A}\crv{X} \crv{B}\crv{Y}]{axby}
          \otimes \rho_{E|\cvec{a}^c\cvec{x}^c \cvec{b}^c\cvec{y}^c}
        \\
      \end{gather}
      using LOPC operations. They can then apply \(\mathfrak{P}'_{\omega}\) to distill key at a rate of at least \(r'\). Therefore, \(r \geq r'\).
              \end{proof}
              ...
              More generally, if the wiring produces an outcome that can be produced by LOPC post-processing of Alice and Bob's Bell test data, then by the definition of \(C_{\sk}\) in \Cref{def:seckeycapbehav}, it cannot increase \(C_{\sk}\). On the other hand, if it creates some correlation between the output of previous Bell test rounds and subsequent rounds, such an increase may be possible.

              Our description of wirings as LOSR operations naturally motivates the question if there is any benefit to be gained from using \emph{private} randomness, unknown to Eve, beyond its necessary role in providing an authenticated classical channel (\Cref{def:diqkdassump}). We distinguish between \emph{shared} private randomness, known to both Alice and Bob, and \emph{local} private randomness known to only one of them.

              TODO local private randomness: noisy preprocessing

              Shared private randomness would constitute a \emph{pre-shared key} (PSK). It is known that, in the device-dependent QKD scenario, a PSK cannot increase the secret key capacity~\cite[Cor. 5.3]{CQKeyDistill}, as a consequence of non-lockability of the secret key capacity (\Cref{sec:diqkd_ustate}). The proof proceeds by arguing that, given a protocol \(\hat{\mathfrak{P}}_{\omega}\) for \(\rho_{ABE}\), if the overall state is \(\rho_{ABEE'}\) rather than \(\rho_{ABE}\), but \(\Tr_{E'}\left[\rho_{ABEE'}\right] = \rho_{ABE}\), executing \(\hat{\mathfrak{P}}_{\omega}\) as if \(E'\) did not exist will achieve
              \begin{equation}
              ...
              Exhibiting an explicit counterexample could be done by showing that a dependent joint distribution \(\prin(x,y)\), which can only be achieved through pre-shared data, can give a higher entropy than any independent distribution, e.g.\ by exhibiting a behaviour such that it is upper bounded for all protocols when using independent distributions, but the lower bound using all inputs exceeds the upper bound.
    TODO Protocol 2 appears to use correlated inputs to increase key rates, not sure if truly increasing capacity and demonstrating lockability

    \Cref{thm:outdep}, and the discussion of correlated inputs from private shared randomness both in this section and \Cref{sec:diqkd_behavsec}, suggests a connection between the advantage of wirings in DIQKD and the \emph{measurement dependent scenario}, first studied in~\cite{RelaxedBell}, where local statistics are described by~\cite[Eq. 2]{MDLBeyondIID}
              \begin{equation}
              ...
              Having established some preliminary facts about wirings, we now turn to analyse the structure of the set of wirings. An important distinction is that the set of \emph{wiring distributions} is \emph{not} the set of \emph{wirings}: we have not proved that any conditional probability that obeys the locality and no-retrocausation conditions, that is, any wiring distribution, constructs a unique wiring, in the sense of an operation that maps one behaviour to another. As it turns out, there may be differences between wiring distributions that do not lead to any observable difference: that is, we could have two wiring distributions \(\chi^{(1)}\) and \(\chi^{(2)}\) such that there are differences in their values, but \(\chi^{(1)}[p] = \chi^{(2)}[p]\) for all behaviours \(p\). We can therefore say that these wiring distributions describe the \emph{same} wiring, and, more generally, that the wiring distribution description contains some redundancy.

              Nonetheless, each wiring distribution constructs \emph{some} wiring, and as seen above, the wiring distribution description allows us to infer important facts about what wirings can achieve. Further, the structure of the space of wiring distributions does provide some information about the structure of the space of wirings. For example, since wirings must be performed locally, the fundamental building block for the space of wirings is set of marginal wirings \(\sW\) that correspond to marginal wiring distributions. Then, abusing notation slightly, we denote by \(\sW^n\) the space of \(n\)-partite wirings, since each party must select a marginal wiring to implement.\footnote{The spaces of marginal wirings for different parties are not the same in general, although one could argue that all of them are subspaces of the version of \(\sW\) with the maximal number of outputs and the maximal number of inputs, as per the argument in \Cref{sec:nl_ns}.}

              We can go even further than this. Physically, a probabilistic wiring must actually execute some deterministic wiring, and its joint wiring distribution is then a convex combination of the deterministic joint wiring distributions, of which there are finitely many. This is the intuition behind the proof of Fine's theorem presented in~\cite{BellNonlocality}, which can be straightforwardly adapted to \(\sW\). Informally, given a decomposition of the behaviour into behaviours that obey the no-retrocausation conditions, these latter behaviours can be converted into deterministic behaviours obeying the conditions by offloading the indeterminism into an auxiliary random variable, and redefining \(\lambda\) to incorporate this auxliary variable.
              ...
              \end{theorem}
              \begin{proof}
                We can decompose a marginal wiring distributions using Bayes' rule as
                \begin{align}
                ...
                \end{align}
                where in the last line we have used the no-retrocausation conditions in \Cref{eqn:wiringnoretro}. Conversely, any distribution of this form, which we term \emph{causal marginal distributions}, satisfies the no-retrocausation conditions
                \begin{gather}
                  \chi_{A|\lambda}(\cvec{x}^k|\cvec{a}^cx) = \sum_{a\cvec{x}^{k+1:c}} \chi_{A|\lambda}(a\cvec{x}^c|\cvec{a}^cx) \\
                  \sum_{a\cvec{x}^{k+1:c}} \chi_{A|\lambda}(a|\cvec{x}^c\cvec{a}^{c}x) \prod_{j=1}^c \chi_{A|\lambda}(x_j|\cvec{x}^{j-1}\cvec{a}^{j-1}x) = \prod_{j=1}^k \chi_{A|\lambda}(x_j|\cvec{x}^{j-1}\cvec{a}^{j-1}x) \\
                  = \chi_{A|\lambda}(a\cvec{x}^k|\cvec{a}^kx).
                \end{gather}
                This tells us that the set of marginal wiring distributions is isomorphic to the set of causal marginal distributions, where, we note, there are no constraints on the conditional distributions that are multiplied together, other than them being valid probability distributions and having the correct input and output cardinalities.

                Consider now a given marginal wiring \(\chi_{A|\lambda}\), decomposed as a causal marginal distribution, and select one of the conditional distributions in the product, which we denote abstractly as \(\chi_{A|\lambda}(s|t)\). Select a particular \(\hat{s}\) and \(\hat{t}\) such that \(\chi_{A|\lambda}(\hat{s}|\hat{t}) = 0 < q < 1\). If no such values exist, the wiring is already deterministic, and we are done. Now, consider wirings \(\chi_{A|\lambda}^{(0)}\) and \(\chi_{A|\lambda}^{(1)}\), such that \(\chi_{A|\lambda}^{(0)} = \chi_{A|\lambda}^{(1)} = \chi_{A|\lambda}\) for all conditional distributions except the one we have identified. For the identified conditional distribution, \(\chi_{A|\lambda}^{(0)}(s|t) = \chi_{A|\lambda}^{(1)}(s|t) = \chi_{A|\lambda}(s|t)\) for all values of \(t\neq\hat{t}\), and for \(t = \hat{t}\), we have
                \begin{align*}
                  \chi_{A|\lambda}^{(0)}(s|\hat{t}) &= \begin{cases}
                    0 & \text{ if } s = \hat{s} \\
                    \frac{1}{1-q} \chi_{A|\lambda}(s|\hat{t}) & \text{ otherwise,} \\
                  \end{cases} \\
                    \chi_{A|\lambda}^{(1)}(s|\hat{t}) &= \begin{cases}
                      1 & \text{ if } s = \hat{s} \\
                      0 & \text{ otherwise.} \\
                    \end{cases}
                    \end{align*}
                    It is easily verified that both \(\chi_{A|\lambda}^{(0)}(s|t)\) and \(\chi_{A|\lambda}^{(1)}(s|t)\) are valid probability distributions, and hence \(\chi_{A|\lambda}^{(0)}\) and \(\chi_{A|\lambda}^{(1)}\) are valid causal marginal distributions and therefore valid wiring distributions. Then, we have \(\chi_{A|\lambda} = (1-q)\chi_{A|\lambda}^{(0)} + q\chi_{A|\lambda}^{(1)}\). We can repeat this process recursively on \(\chi_{A|\lambda}^{(0)}\) and \(\chi_{A|\lambda}^{(1)}\) until \(\chi_{A|\lambda}\) is decomposed completely into a weighted sum of deterministic wiring distributions
                    \[ \chi_{A|\lambda} = \sum_{w=1}^m q_w \chi_{A,w}. \]
                    ...
                    \begin{equation}
                      \chi(ab\cvec{x}^c\cvec{y}^c|\cvec{a}^c\cvec{b}^cxy) = \sum_{\lambda w} P_{\Lambda'}(\lambda) \chi_{A,w}(a\cvec{x}^c|\cvec{a}^cx) \chi_{B|\lambda}(b\cvec{y}^c|\cvec{b}^cy),
                    \end{equation}
                    where now all \(\chi_{A,w}\) are deterministic. The same process can be used to decompose Bob's, or any other party's, marginal wiring distribution into a convex combination of deterministic wirings.
              ...
              In order to illustrate one of the redundancies that emerges from the naive approach, we use a different, more operational description for deterministic wirings.  For deterministic wirings, we can consider her inputs \(\cvec{x}^c\) to her boxes as being computed by a sequence of \(c\) deterministic functions with range \(\dintv{1}{i_A}\):
              ...
              We refer to each function \(W_{A,\lambda}^{(j)}\) as a \emph{marginal wiring input function}, and \(T_{A,\lambda}\) as the \emph{marginal wiring output function}, and all of these functions as \emph{marginal wiring functions}. The corresponding marginal wiring distribution \(\chi_{A|\lambda}\) is then
              \begin{equation}
                \chi_{A|\lambda}(a\cvec{x}^c|\cvec{a}^cx) = \indic{a = T_{A,\lambda}(\cvec{x}^{c}, \cvec{a}^{c}, x)} \prod_{j=1}^c \indic{x_j = W^{(j)}_{A,\lambda}(\cvec{x}^{j-1}, \cvec{a}^{j-1}, x)}.
              \end{equation}
            ...
            \caption[Representation of behaviours that violate CHSH inequality]{Representation of behaviours that violate \(S \leq 2\), with CHSH facet in blue}%
                  ...
                  A possible approach is to introduce the joint wiring distribution \(\chi(ab\cvec{x}^c\cvec{y}^c|\cvec{a}^c\cvec{b}^cxy)\) as an optimisation variable into the semidefinite program in \Cref{eqn:bff_npa}, or more specifically \(\chi_{A|\lambda}(a\cvec{x}^c|\cvec{a}^cx\) and \(\chi_{B|\lambda}(b\cvec{y}^c|\cvec{b}^cy)\), as defined in \Cref{eqn:mwirdistdef}. Since \(\chi\) takes a discrete set of arguments, its value for every possible argument can be introduced as a variable, and constrained with the no-retrocausation conditions in \Cref{eqn:wiringnoretro}, and the appropriate normalisation:
                  \begin{equation}
                    \begin{aligned}[c]
                      \sum_{a\cvec{x}^c} \chi_{A|\lambda}(a\cvec{x}^c|\cvec{a}^cx) &= 1 \\
                      \sum_{b\cvec{y}^c} \chi_{B|\lambda}(b\cvec{y}^c|\cvec{b}^cy) &= 1.
                    \end{aligned}
                  \end{equation}
                  ...
                  \end{equation}
                  Therefore, the maximum is achieved on the vertices of the polytope, TODO just use result on convex functions
                  ...
                  \begin{align}
                    P_{\crv{V_1V_2}}(v_1v_2) &= \sum_{e} \prin(x,y) \sum_k m_{a|x}^{(k_A)} n_{b|y}^{(k_B)} o_e^{(k_E)} p^{(k)} \\
                                             &= \prin(x,y) \sum_k m_{a|x}^{(k_A)} n_{b|y}^{(k_B)} \sum_e o_e^{(k_E)} p^{(k)} \\
                                             &= \prin(x,y) \sum_k m_{a|x}^{(k_A)} n_{b|y}^{(k_B)} p^{(k)} \\
                    P_{\crv{V_2E}}(v_2e) &= \sum_{ax} \prin(x,y) \sum_k m_{a|x}^{(k_A)} n_{b|y}^{(k_B)} o_e^{(k_E)} p^{(k)} \\
                                         &= \sum_{x} \prin(x,y) \sum_k n_{b|y}^{(k_B)} o_e^{(k_E)} \sum_a m_{a|x}^{(k_A)} p^{(k)} \\
                                         &= \prin(y) \sum_k n_{b|y}^{(k_B)} o_e^{(k_E)} p^{(k)} \\
                    P_{\crv{E}}(e) &= \sum_{by} \prin(y) \sum_k n_{b|y}^{(k_B)} o_e^{(k_E)} p^{(k)} \\
                                   &= \sum_k o_e^{(k_E)} p^{(k)}.
                  \end{align}
      ...
      \end{equation}
                  ...
                  Markov chains
                    ...
                    This is an important result, because later work has shown that the CHSH values of isotropic boxes are not distillable without shared randomness~\cite{NLMonotones}, and we discuss the possibility of proving this even with shared randomness in \Cref{sec:wirmono_isodist}, giving hope that we can close a problem that has been open for over a decade~\cite{NLLimits, DistillationBounds}.
