  ...
  \end{defn}
    ...
    \begin{equation}
      ...
      & -\eta_B(1-\eta_A)\angleb{\tilde{B}_y} + (1-\eta_A\eta_B) )
              ...
              \begin{equation}
                \begin{aligned}[c]
                  \chi(abx_0y_0|a_0b_0xy) = \sum_{\lambda} P_{\Lambda}(\lambda) \chi_A(ax_0|a_0x\lambda) \chi_B(by_0|b_0y\lambda),
                \end{aligned}
              \end{equation}
              ...
              \begin{gather}
                ...
                \chi(ab\cvec{x}^c\cvec{y}^c|\cvec{a}^c\cvec{b}^cxy) = \sum_{\lambda} P_{\Lambda}(\lambda) \chi_A(a\cvec{x}^c|\cvec{a}^cx \lambda) \chi_B(b\cvec{y}^c|\cvec{b}^cy \lambda) \label{eqn:mwirdistdef}
              \end{gather}
              ...
              These conditions will hopefully simplify the space of wirings, since without the conditions, we would have to handle \({(o_A i_A^c)}^{i_A^c o_A} {(o_B i_B^c)}^{i_B^c o_B}\) vertices in \(\Ls\) (\(\approx \num{5.12e18}\) for the QKD setting). We will shortly turn our attention to characterising this space.
                ...
                Note that, while the definitions and discussion thus far focus on the bipartite case, they generalise readily to the \(n\)-partite case. We will not discuss this case in detail, but will occasionally mention how this generalisation can be done. Further, the definition of wirings in \Cref{eqn:jwirdistdef} is expressed in terms of \(p^c(\cvec{a}^c\cvec{b}^c|\cvec{x}^c\cvec{y}^c)\), the joint behaviour of the \(c\) boxes. This very general formulation accomodates situations where the individual boxes are not simply iid copies of each other, or where the order of the boxes is permuted, so that, for independent boxes, we have most generally
                  ...
                  TODO permutation doesn't affect general joint behaviours but we have to indicate the permutation between the order specified (followed by behaviour) and the order used (followed by wirings, required for no-retrocausation)
              ...
              \begin{proof}
                ...
                \begin{align}
                  \chi(\cvec{x}^c\cvec{y}^c|xy) &= \sum_{ab} P_{\Lambda}(\lambda) \chi(ab\cvec{x}^c\cvec{y}^c|\cvec{a}^c\cvec{b}^cxy) \\
                                                &= \sum_{ab} \sum_{\lambda} P_{\Lambda}(\lambda) \chi_A(a\cvec{x}^c|\cvec{a}^cx \lambda) \chi_B(b\cvec{y}^c|\cvec{b}^cy \lambda) \\
                                                &= \sum_{\lambda} P_{\Lambda}(\lambda) \chi_A(\cvec{x}^c|\cvec{a}^cx \lambda) \chi_B(\cvec{y}^c|\cvec{b}^cy \lambda)  \\
                                                &= \sum_{\lambda} P_{\Lambda}(\lambda) \chi_A(\cvec{x}^c|x \lambda) \chi_B(\cvec{y}^c|y \lambda),\label{eqn:nooutputmwir}
                \end{align}
              ...
              \begin{gather}
                p'(ab|xy) = \sum_{\cvec{a}^c\cvec{b}^c\cvec{x}^c\cvec{y}^c} \sum_{\lambda} \chi_A(a\cvec{x}^c|\cvec{a}^cx \lambda) \chi_B(b\pi(\cvec{y}^c)|\pi(\cvec{b}^c)y \lambda) P_{\Lambda}(\lambda) p^c(\cvec{a}^c\cvec{b}^c|\cvec{x}^c\cvec{y}^c) \\
                = \sum_{\cvec{a}^c\cvec{b}^c\cvec{x}^c\cvec{y}^c} \sum_{\lambda} \chi_A(a\cvec{x}^c|\cvec{a}^cx \lambda) \chi_B(b\pi(\cvec{y}^c)|\pi(\cvec{b}^c)y \lambda) P_{\Lambda}(\lambda) \Tr\left[\rho_{AB} \left( M_{\cvec{a}^c|\cvec{x}^c} \otimes N_{\cvec{b}^c|\cvec{y}^c} \right) \right], \\
              ...
              \end{align}
               ...
               TODO explicitly construct expanded Hilbert space to avoid dealing with shared randomness

               Let \(C_{\sk}(p') = r'\), and let \(\rho_{\crv{A}^c\crv{X}^c \crv{B}^c\crv{Y}^c E}\) be the qqq state compatible with \(p\) with the best possible \(p_{\mathrm{in}}^c\) for Alice and Bob, and the best possible \(E\) for Eve, while \(\rho'_{\crv{A}\crv{X} \crv{B}\crv{Y} E}\) is the ccq state with the best possible \(p_{\mathrm{in}}\) for Alice and Bob and the implementation above (hence the same Hilbert space \(E\) used in both), with the Hilbert space expanded to remove the shared randomness \(\lambda\). Then we can write these states as
      \begin{align}
        \rho_{\crv{A}^c\crv{X}^c \crv{B}^c\crv{Y}^c E} &= \sum_{\cvec{a}^c\cvec{b}^c \cvec{x}^c\cvec{y}^c} \begin{aligned}[c]
          & p^c_{\rm in}(\cvec{x}^c,\cvec{y}^c) p^c(\cvec{a}^c\cvec{b}^c|\cvec{x}^c\cvec{y}^c) \proj[\crv{A}^c\crv{X}^c \crv{B}^c\crv{Y}^c]{\cvec{a}^c\cvec{x}^c\cvec{b}^c\cvec{y}^c} \\
        ...
        \end{aligned} \\
          \rho'_{\crv{A}\crv{X} \crv{B}\crv{Y} E} &= \sum_{abxy} p'_{\rm in}(x,y) \proj[\crv{A}\crv{X} \crv{B}\crv{Y}]{axby} \otimes \Tr_{AB} \left[ \rho_{ABE} \left(M'_{a|x} \otimes N'_{b|y} \otimes I_E\right) \right] \\
                                                  ...
                                                  \begin{aligned}[c]
          & P_{\Lambda}(\lambda) \chi_A(a\cvec{x}^c|\cvec{a}^cx \lambda) \chi_B(b\pi(\cvec{y}^c)|\pi(\cvec{b}^c)y \lambda) \\
      ...
      \begin{equation}
        p^c_{\rm in}(\cvec{x}^c,\cvec{y}^c) = \sum_{xy} \chi(\cvec{x}^c\cvec{y}^c|xy)p'_{\rm in}(x,y) = \sum_{xy} \chi(\cvec{x}^c\cvec{y}^c|\cvec{a}^c\cvec{b}^cxy)p'_{\rm in}(x,y),
      \end{equation}
      and local operators
      \begin{align}
      ...
      \begin{gather}
        ...
        =   \sum_{\cvec{a}^c\cvec{b}^c \cvec{x}^c\cvec{y}^c}
          p^c_{\rm in}(\cvec{x}^c,\cvec{y}^c) p^c(\cvec{a}^c\cvec{b}^c|\cvec{x}^c\cvec{y}^c)
      ...
      using LOPC operations. They can then apply \(\mathfrak{P}'_{\omega}\) to distill key at a rate of at least \(r'\). Therefore, \(r \geq r'\).
              ...
              \end{theorem}
              \begin{proof}
                From \Cref{eqn:wiringnoretro}, any marginal wiring 
              \end{proof}
