...
\item[]}{\end{list}}
  ...
  \begin{align}
    ...
    \begin{align*}
      ...
      This precise definition of wiring allows us to state a simple no-go theorem.
      \begin{theorem}[No Fundamental One-Way Improvement from Wiring]
        Wiring cannot increase the Devetak-Winter key rate. Therefore, it can only be used to achieve the Devetak-Winter key rate if restricted to one-way settings. However, it may increase the key rate in two-way settings, so it may increase the secret key capacity overall. 
      \end{theorem}
      \begin{proof}
        Consider the Devetak-Winter key rate evaluated using all Bell test data and normalised:
        \[ r_{\DW} = \frac{H(A^n|E^n) - H(A^n|B^n)}{H(A^n)}. \]
        Wiring involves the application of a deterministic function, possibly selected over public communication. Deterministic actions cannot change the entropy, and since the shared randomness is given to Eve as well, both conditional entropies decrease by the same amount. Formally, if \(D\) holds the choice of wiring,
        \begin{align*}
          r'_{\DW} &= \frac{H(A^n|E^n D) - H(A^n|B^n D)}{H(A^n)} \\
                   &= \frac{H(A^n D|E^n) - H(D) - H(A^n D|B^n) + H(D) }{H(A^n)} \\
                   &= \frac{H(A^n D|E^n) - H(A^n D|B^n)}{H(A^n)} \\
        \end{align*}
      \end{proof}

        TODO Does quantum conditioning invalidate this? What role does E/AB-lockability play? There is a gap between DIQKD and QKD rates---does this change its lockability?

        TODO two-way comms, improve understanding of protocol design
