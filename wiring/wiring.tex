...
\item[]}{\end{list}}
  ...
  \begin{align}
    ...
    \begin{align*}
      ...
      \end{equation}
      which we refer to as the \emph{marginal wiring functions}. If we focus only on \(j = c\), we can see these distributions as elements of \(\Ls\), with outputs \((a, \cvec{x}^c)\) and \((b, \cvec{y}^c)\), and inputs \((\cvec{a}^{c-1}, x)\) and \((\cvec{b}^{c-1}, y)\). However, unlike the simple LOSR case for a single box, the space of possible distributions \(\chi\) may be a strict subset of \(\Ls\), due to the additional no-retrocausation conditions for \(j < c\). This will hopefully simplify the space of wirings, since it would otherwise be a polytope with \({(o_A i_A^c)}^{i_A^c o_A} {(o_B i_B^c)}^{i_B^c o_B}\) vertices (\(\approx \num{5.12e18}\) for the QKD setting). We next turn our attention to characterising this space.

      \subsection{Generating the Wiring Polytope}

      Physically, a probabilistic wiring must actually execute some deterministic wiring, and its joint wiring function is then a probability distribution over the deterministic joint wiring functions, of which there are finitely many. This is the intuition behind the proof of Fine's theorem, which can be straightforwardly adapted to this polytope. Therefore, to determine the polytope of joint wiring functions, we can use the same approach as is typically used for \(\Ls\), where deterministic operations are taken as the vertices of the polytope, and the facet-defining inequalities derived from there. 

      For ease of implementation, we use a different, more operational description for deterministic marginal wiring functions. Focusing on Alice's marginal wiring functions, her inputs \(\cvec{x}^c\) to her boxes as being computed by a sequence of \(c\) functions with range \(\dintv{1}{i_A}\):
      ...
      We refer to this set of functions as a \emph{marginal wiring map}. The corresponding marginal wiring function \(\chi_A\) is then
      \begin{equation}
      ...
      We refer to the discrete set of deterministic marginal wiring functions that obey these relations as \(\sW_D\), and the polytope of joint wirings \(\sW\) is then the convex polytope with vertices \(\sW_D \times \sW_D\). We also define \(\sWB(p)\) as the set of behaviours generated by applying \(\sW\) to a given behaviour \(p\) using \Cref{eqn:wiringfndef}.

      Using our classification for \(\sW_D\), simple combinatorics gives us \(i_A^{j} o_A^{j-1}\) possible inputs for \(W^{(j)}_{A,\lambda}\). However, since the wiring is deterministic, \(x_j\) is completely fixed by \(\cvec{x}^{j-1}\), \(\cvec{a}^{j-1}\) and \(x\). Therefore, the only free input is \(x\), and so there are only \(i_A o_A^{j-1}\) valid inputs. Therefore, the number of possible \(W^{(j)}_{A,\lambda}\) is \(\exp_{i_A}(i_A o_A^{j-1})\). Similarly, the number of possible \(W_{A,\lambda}\) is \(\exp_{o_A}(i_A o_A^{c})\). The total number of wirings is then
      \begin{equation}
      ...
      Unfortunately, with just two boxes, there are 16384 marginal wirings for Alice, and \(\approx \num{8.06e7}\) for Bob, for a total of \(\approx \num{1.32e12}\). Fixing the AND gating as discussed above gives us 128 wirings for Alice and 186624 for Bob, for a total of \(\approx \num{2.39e7}\). Both are much more manageable than the number of vertices of \(\Ls\) without causality constraints. Still, this enumeration of the possible wirings is naive, and does not attempt to use symmetry arguments to remove degeneracy or wirings that will not be helpful. Based on the symmetry considerations in \Cref{sec:qkdset}, we should be able to confine ourselves to a smaller region of the polytope.
