...
\newcommand{\id}{\mathrm{id}} % identity morphism
  ...
  \begin{align}
    ...
    \begin{align*}
      ...
      Using the Pironio key rate, we are able to find \emph{revival} in certain parameter regimes: behaviours with \(r_P = 0\), but which have \(r_P > 0\) after wiring. In \Cref{fig:exptplt}, the N-AND wiring is able to achieve \(r_P > 0\) in parameter regimes where \(r_P = 0\) before wiring. Our objective, then, is to understand and generalise this phenomenon.

  \begin{figure}
    \centering
    \includegraphics[width=\linewidth]{exptplt.pdf}
    \caption{\label{fig:exptplt} Minimum \(\eta\) required for \(r_P > 0\), given \(n_c\).}
  \end{figure}
