...
\item[]}{\end{list}}
  ...
  \begin{align}
    ...
    where the systems \(A\), \(B\) and \(E\) are Alice's, Bob's and Eve's systems, respectively, in an individual Bell test round. This data is derived from measurements on a \emph{qqq} state \(\rho_{Q_A Q_B E}\). The measurements on \(Q_A\) and \(Q_B\) are characterised by the POVMs  \({\{M_{a|x}\}}_{a}\) and \({\{N_{b|y}\}}_{b}\) respectively, \({\{M_{a|x}\}}_{a}\) corresponding to output \(a\) and \({\{N_{b|y}\}}_{b}\) corresponding to output \(b\). The behaviour is then given by
    ...
    \begin{align*}
      ...
      Using the CHSH key rate, we are able to find \emph{revival} in certain parameter regimes: behaviours with \(r_S(p) = 0\), but which have \(r_S(p_N) > 0\), as shown in \Cref{fig:exptplt}. However, since wiring no longer treats the individual rounds as i.i.d., it should not be used as part of a protocol for the purpose of computing \(C_{\sk}\). Therefore, it is possible that \(C_{\sk}(p_N) > C_{\sk}(p)\). However, it might just be that \(r_S(p) < C_{\sk}(p)\), and that the wiring does not improve the secret key capacity. We can now state one of the aims of our research more precisely:
      ...
      % TODO something fishy here: all wirings can actually be reversed, so
      % maybe shared randomness should be allowed?
      ...
      \begin{equation}\label{eq:wiringdef}
        \chi(ab\cvec{x}\cvec{y}|\cvec{a}\cvec{b}xy) = \chi_A(a\cvec{x}|\cvec{a}x) \chi_B(b\cvec{y}|\cvec{b}y), \chi_A, \chi_B \in \{0,1\}\; \forall a,\cvec{x},\cvec{a},x,b,\cvec{y},\cvec{b},y
      \end{equation}
      where \(\cvec{a}\) is the vector of all the outputs from Alice's boxes that are wired together, analogously for \(\cvec{b}\), \(\cvec{x}\) and \(\cvec{y}\), and the condition that \(\chi_A, \chi_B \in \{0,1\}\) enforces that the wirings are deterministic. That this set is discrete opens the possibility of a brute-force exhaustive search through all possible wirings in order to find the one which optimises the key rate.

    In fact, we can restrict the set of possible wirings even further with causality constraints. Let each player have \(c\) boxes, and let subscripts denote the order in which the boxes are used. Then, Alice's inputs \({\{x_i\}}_{i=1}^c\) are computed by a sequence of \(c\) functions with range \(\dintv{1}{i_A}\):
      ...
      \subsection{Classifying Wirings}\label{sec:locwir_class}
      ...
      \begin{table}
        ...
        \begin{minipage}{0.5\linewidth}
          ...
          \end{center}
        \end{minipage}
        \caption{Left: Lookup table for \(C_A^{(2)}\), giving \(x_2\). Right: Lookup table for \(C_A\), giving overall output \(a\). Entries forbidden by \(C_A^{(2)}\) are marked with X (don't care). Both are written as Karnaugh maps, with the row and column labels in Gray code order.}\label{tab:wiring_lut}
      \end{table}
      ...
      Given that our set of transformations is discrete, however, this space would also be discrete. Using a continuous relaxation for the binary variables \(\chi_A\) and \(\chi_B\), which would represent the use of non-deterministic wirings, it might be possible to find the convex hull of the achievable behaviours. Then, if we have a bound on the key rate which is convex on the parameters of that space of behaviours, it might be possible to optimise it efficiently over that space, instead of performing a brute-force search. Even if the optimum can only be achieved by a convex combination of wirings, this would still be a bound on the achievable key rates. Alternatively, we could keep the restriction to binary variables and use techniques for binary optimisation, but it is unclear if this can be done efficiently.

      \begin{question}\label{qn:wirspace}
        Are there functions that can be efficiently optimised over this space which provide bounds on the key rate, either with binary variables or with a continuous relaxation? Will an optimum found in the continuous relaxation be close to an actual optimum?
      \end{question}
      ...
      Nonetheless, a brute force enumeration can answer \Cref{fqn:wircap} in the affirmative, if we find a behaviour with \(C_{\sk} = 0\) but which exhibits \(C_{\sk} \geq 0\), that is, with an upper bound of 0 on the key rate before wiring, and a lower bound \(\geq 0\). Finding a concrete example of this phenomenon will motivate a deeper analysis of the underlying structures.
      ...
      One recipe for constructing upper bounds is to optimise an upper bound for the secret key capacity of quantum strategies (that is, for states and measurements) over the set of strategies that are consistent with the observed behaviour, which would be a direct application of the definition of \(C_{\sk}\) in \Cref{eqn:seckeycap}. Even if this recipe is not followed precisely, most upper bounds depend intimately on upper bounds for strategies. Therefore, we will first review the upper bounds for quantum strategies, before summarising the upper bounds for behaviours.
      ...
      \subsection{Upper Bounds for Strategies}

      \subsection{Upper Bounds for Behaviours}

      There has been a recent flurry of research related to upper bounds on behaviours~\cite{NotSufficient, RevisedPeres, DIQKD_QKD_Gap, CCSquashedEntangle, DIQKD_Limits}. Fortunately,~\cite{CCSquashedEntangle} 

      A notable non-faithful upper bound is given in~\cite{NotSufficient}, where it is shown that \emph{Werner states}
      \begin{equation}
        \rho_{AB}^v = v{\ket{\psi_-}\bra{\psi_-}}_{AB} + \frac{1-v}{4}I_{AB},
      \end{equation}
      with \(\ket{\psi_-} = (\ket{01} - \ket{10})/\sqrt{2}\)
    ...
    % TODO quasi-relative entropies for REE?
