...
\newcommand{\id}{\mathrm{id}} % identity morphism
  ...
  As this is still an interim report, we do not have many concrete results, although we have investigated and tried to understand a variety of techniques for approaching this problem. We first review the basic, relevant notions of nonlocality and DIQKD in \Cref{sec:pre}, then proceed to review some preliminary results in \Cref{sec:preres}. Next, we examine the space of possible behaviours relevant to our problem in \Cref{sec:qkdset}, and we discuss how we can perform an exhaustive search through the space of possible wirings in \Cref{sec:locwir}. We then review and compare the techniques we have studied for attacking this problem, namely upper bounds (\Cref{sec:ubound}) and lower bounds (\Cref{sec:lbound}) for DIQKD key rates, and functions that are monotones under local wiring in \Cref{sec:nlmono}
  ...
  \begin{align}
    ...
    Much of the effort in bounding the DIQKD key rate has focused on this definition, and this is the setting we will work in. While \({H(A|B)}_{\rho}\) can be computed directly as the QBER, upper and lower bounds for \({H(A|E)}_{\rho}\) are much harder to find.
    ...
    \begin{equation}
      r_{DW} \geq r_{P} = \max \left\{ 0, 1 - h_2(Q) - h_2\left( \frac{1 + \sqrt{{(S/2)}^2-1}}{2} \right) \right\},
    \end{equation}
    where we refer to \(r_P\) as the \emph{Pironio key rate}. Being the rate for an explicit, achievable protocol, this is a lower bound for the total secrecy extractable. However, because the proof of the bound involves explicitly constructing the states and measurements that gives the greatest advantage to Eve while still producing the observed value of \(S\), it is also an upper bound for key rates based only on \(S\), since any observed \(S\) could have been produced by those states.

    \section{Preliminary Results}\label{sec:preres}

    Our preliminary results found a revival of the Pironio key rate for states that are generated according to a simple experimental model. We will be using it to illustrate the relevant issues and directions we wish to pursue in our analysis. In this model, we denote the density matrix of the state as \(\rho_{AB}\) and observables as \(\tilde{A}_x\) and \(\tilde{B}_y\), for a perfect experimental setup, and let the Hilbert spaces of systems \(A\) and \(B\) be \(d_A\) and \(d_B\) respectively.
    ...
    \begin{align*}
      ...
      Using the Pironio key rate, we are able to find \emph{revival} in certain parameter regimes: behaviours with \(r_P = 0\), but which have \(r_P > 0\) after wiring.
      ...
      Nonetheless, finding an analogous linear structure for the space of possible wirings might be helpful. However, there are serious difficulties with this idea. The strength of this linear characterisation of the possible couplers was that any coupler inside the polytope could be expressed as a convex combination of the extremal points, that is, a probabilistic mixture of the deterministic wirings corresponding to those points. Therefore, it is possible to continuously transform the resultant behaviour from that produced by one coupler to another.

      Since the wirings are done locally by Alice and Bob, the choice of wiring in each round will be determined by local randomness. In the similar technique of \emph{noisy preprocessing}, Alice flips her bits based on her local randomness with a probability \(q\), which has been shown to improve key rates~\cite{AsymmetricCHSH}. Now, if Alice and Bob use a non-deterministic wiring, and the probability of them selecting wirings that give different results, averaged over all rounds, is \(w\), the resultant behaviour would be identical to the behaviour from noisy preprocessing with \(w = q\). Therefore, there is no need to model non-deterministic wirings, since the same effect can be achieved by the established technique of noisy preprocessing.

      Identifying the extremal points of such a polytope, if it even exists, would also not be helpful, since there is no \emph{a priori} reason why the extremal points should produce the behaviour with the highest key rate. As neither the linear structure of the polytope or its extremal points would be helpful for us, we will not pursue this line of investigation.

      Instead, we will characterise each wiring in the following way. Let each player have \(c\) boxes, and let subscripts denote the order in which the boxes are used. Then, Alice's wiring can be represented by a sequence of \(c\) functions with range \(\{1..i_A\}\)
      \begin{equation} x_1 = x \end{equation}
      \begin{equation} x_j = C^{(j)}_A(x_1, \ldots, x_{j-1}, a_1, \ldots, a_{j-1}),\,j \in \{2..c\} \end{equation}
