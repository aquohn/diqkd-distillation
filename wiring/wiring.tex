...
\usepackage{microtype}  % fine-tuned typographic adjustments
...
\item[]}{\end{list}}
  ...
  \pgfplotsset{%
    ...
    \begin{defn}[\(\epsilon\)-security]
      ...
      \end{equation}
      where \(\Trdist{\cdot}{\cdot}\) is the trace distance (\Cref{sec:prelim_infot}), \(\tau_{\crv{K}_{A}^{\ell_n} \crv{K}_{B}^{\ell_n}}\) is the \emph{ideal key}
      \begin{equation}
    ...
    Observe that, given such a state, Alice and Bob can locally generate (or generate from a pre-shared key; as discussed in \Cref{sec:diqkd_proto} and elaborated on in \Cref{sec:diqkd_lock}) their measurement inputs according to a desired distribution \(\prin\) in registers \(\crv{X}\) and \(\crv{Y}\), and measure the resulting \(\rho_{A\crv{X}B\crv{Y}E}\) to produce \(\rho_{\crv{AXBY}E}\). Since these are valid local operations in a device-dependent protocol, device-independent protocols are a subset of device-dependent protocols, and the device-dependent key rate must be an upper bound for the device-independent key rate of the behaviour. We formalise this below.
    ...
    \begin{defn}[QKD Key Rate]
      ...
      \end{equation}
    \end{defn}
    Here, \(L_{\Omega}\) is the secrecy cost of the state \(\rho\): the PSK expenditure required to ensure that Alice, Bob and Eve share the state \(\rho\). It generalises \(L_{\Omega}(\prin^n)\) in \Cref{def:diqkdkr}.
    ...
    \begin{defn}[The Device-Dependent Secret Key Capacity]\label{def:seckeycapstate}
      ...
      \end{equation}
      where \(\qproto{\Omega}\) denotes a (possibly) device-dependent protocol (such that local operations are no longer constrained to be classical), maximised over all QKD protocols that produce \(\epsilon\)-secure keys. The definition of the set of QKD protocols is identical to \Cref{def:diqkdproto}, except quantum LOPC operations are allowed (that is, general CPTP maps, not just classical stochastic ones).
    \end{defn}
    ...
    Since this proof applies to \emph{any} state and \emph{any} protocol, however, it also applies to DIQKD protocols. This is the reason why the set of general ccq states (\Cref{def:ccq_belltest}) is defined without a PSK\@: the protocol non-lockability construction could be applied to any protocol using a ccq state and a PSK to construct a protocol that is at least as good and does not use the PSK\@.
    ...
    TODO we assume that the PSK is kept secret while quantum communication is ongoing. The quantum state will be fixed by then.
    ...
    \begin{table}
      \begin{Tabular}{| >{\centering}m{\tableline{0.3}}|m{\tableline{0.7}}|}
        \hline
        \( E^{cc,(x^{\star},y^{\star})}_{sq,par} \leq \LCxE{\{I_{AL}, I_{FBL+}\}} \) & For any protocol that uses one key-generating basis and the statistics \(Q\) and \(\hat{S}\), \(I_{AL}\) is an upper bound for intrinsic information computed in~\cite{RevisedPeres}. For any protocol that broadcasts the input settings, \(I_{FBL+}\) is the non-faithful upper bound on the key rate from~\cite{NotSufficient} for any behaviour that can be generated from Werner states with projective measurements. However, \(E^{cc,(x^{\star},y^{\star})}_{sq,par}\) is a convex upper bound for the key rate when the protocol and behaviour satisfy both sets of conditions, and a lower bound for the lower convex envelope of these quantities. \\ \hline
        \( E^{cc}_{sq,dev} \leq I_{FBL+} \) & For any protocol that broadcasts the input settings, \(I_{FBL+}\) is the non-faithful upper bound on the key rate from~\cite{NotSufficient} for any behaviour that can be generated from Werner states with projective measurements. However, \(E^{cc}_{sq,dev}\) is a convex upper bound for the key rate of such protocols, and a lower bound for \(I_{FBL+}\) for such behaviours. \\ \hline
      \end{Tabular}
      \caption{\label{tab:ccsqcomp} Comparison between bounds from cc-squashed entanglement and earlier results.}
    \end{table}

    Once again, it is fortunate that all optimisations are infima, because this is a difficult optimisation to compute. Although it was applied to bound specific classes of behaviours and protocols, as detailed in \Cref{tab:ccsqcomp}, it might be interesting to see if it could have more general applicability.
    ...
    The full derivation of this result involves the use of the operator algebra approach to quantum mechanics and spectral theory, which is generally not covered in undergraduate treatments of quantum mechanics and quantum information. An important result from their general approach is the extension to bounded infinite-dimensional operators. This has been reviewed (using more elementary methods, without the generality of operator algebras) in \Cref{sec:qretech}, but for this section, we write the expressions in finite-dimensional form, where all spectra are discrete.
    ...
    \begin{equation}
      ...
      & -\eta_B(1-\eta_A)\angleb{\tilde{B}_y} + (1-\eta_A\eta_B) )
              ...
              \end{equation}
              We also define \(\sWB_c(p^c)\) as the set of behaviours generated by applying \(\sW_c^n\) to an \(n\)-partite, \(c\)-round behaviour \(p^c\) using \Cref{eqn:jwirdistdef}.
              ...
              \]
              is represented by the lookup tables in~\Cref{tab:wiring_lut}, where juxtaposition is the Boolean AND, addition is the Boolean OR, and the overbar is the Boolean NOT.\ \(W_{A}^{(2)}\) and \(T_{A}\) are fully specified by 4 and 8 values respectively, instead of the full 8 and 32 values that might be naively expected.
              ...
              \begin{equation}
                ...
                \begin{align*}
                  ...
                  \textquote[{\cite[End. 19]{ShortEntangleSwap}}]{Note that we do not give the coupler an input, as we intend it to correspond to a specific measurement (analogous to the Bell measurement in quantum mechanics), whereas the standard box inputs correspond to a selection of possible measurements.}
...
Any[]
          ...
          & P_{\Lambda}(\lambda) \chi_{A|\lambda}(a\cvec{x}|^c\cvec{a}^cx) \chi_{B|\lambda}(b\cvec{y}^c|\cvec{b}^cy) p^c(\cvec{a}^c\cvec{b}^c|\cvec{x}^c\cvec{y}^c) \rho_{E^c|\cvec{a}^c\cvec{x}^c \cvec{b}^c\cvec{y}^c\lambda}.
                        ...
                        However, this protocol is equivalent to applying the stochastic maps described above to \(\rho\) with \(\prin[\hat{p}] = \prin'\), followed by \(\proto[\ell']{\Omega'}\), which would be one of the protocols optimised over in the definition of \(\hat{r}\) (\Cref{eqn:hatrdef}). Therefore,
                      ...
                      More generally, if a wiring produces a behaviour \(p'\) that can be produced by LOPC post-processing of Alice and Bob's Bell test data on \(p^c\), then by \Cref{def:seckeycapbehaviid}, we cannot have \(C_{\sk}^{\otimes}(p') \geq C_{\sk}^{\otimes}(p^c)\). On the other hand, if the wiring creates some correlation between the output of previous Bell test rounds and subsequent rounds, such an increase may be possible.
                      ...
                      Shared private randomness cannot be so easily dismissed. From the discussion in \Cref{sec:diqkd_lock}, the effect of using shared private randomness \emph{before} the state is distributed cannot be ruled out by a lockability-type argument. It is also not quite equivalent to the use of a PSK for generating the input distribution, since the overall effect of the wiring would depend on the outputs as well. By \Cref{thm:feedforward}, we can only see a benefit from considering wirings that interact with the outputs in a nontrivial way, but this would be more complicated to analyse. Therefore, we leave this question for future work, and focus on results that can be obtained without considering such wirings.
                      \begin{question}
          ...
          & \nu_{\crv{A}} \E_{\crv{A}}[{(\E_{\crv{B}|\crv{A}}[f(\crv{A},\crv{B})])}^2] \\ + & \nu_{\crv{B}} \E_{\crv{B}}[{(\E_{\crv{A}|\crv{B}}[f(\crv{A},\crv{B})])}^2] \\
                        ...
                        In order to parametrise states and measurements, we work with a parametrisation of \(d\)-dimensional unitary operators in terms of \(d^2\) angles \(\{\theta_{1,1},\ldots,\theta_{d,d}\}\)~\cite{CompositeParam}:
                        \begin{equation}
                        ...
                        \begin{equation}\label{eqn:unitaryparam_Theta}
                          \Theta_{j,k} = \begin{aligned}
                          & \cos(\theta_{j,k}) & \ket{j}\bra{j} & - \sin(\theta_{j,k}) & \ket{j}\bra{k} \\
                            + & e^{i\theta_{k,j}}\sin(\theta_{j,k}) & \ket{k}\bra{j} & + e^{i\theta_{k,j}}\cos(\theta_{j,k}) & \ket{k}\bra{k} \\
                            + & \sum_{l \neq j, k} \ket{l}\bra{l} & & &
                          \end{aligned}
                        \end{equation}
                        ...
                        \begin{equation}\label{eqn:unitaryparam_phase}
                        ...
                        TODO this needs to be fixed; define \(\bar{M}_a\) such that \(\bar{M}_a^{\dagger} \bar{M}_a = M_a\), and note that there is unitary freedom in the definition: \(\bar{M}_a^{\dagger} U^{\dagger} U \bar{M}_a = M_a\).
                        ...
                        \end{equation}
                        where we have used \(\sqrt{M_a}^{\dagger} = \sqrt{M_a}\), the columns of \(V\) are a set of \(d\) orthonormal vectors in \(\Hs\). Therefore, the sequence of operators \((M_a)_{a=1}^o\) forming a POVM induces a unique set of \(d\) orthonormal vectors in \(\Hs\).

                        Conversely, any orthonormal set of \(d\) vectors \({\{\ket{\gamma^{(g)}}_{\Hs}\}}_{g=1}^d\) induces a unique valid POVM on \(A\). We decompose each state as \(\ket{\gamma^{(g)}}_{\Hs} = \oplus_{a=1}^o g_a \ket{\gamma^{(g)}_a}\), with normalisation factors \(g_a \in \R_{+}\), and construct \(o\) \(d\times{d}\) operators
                        \begin{equation}
                        ...
                        \end{equation}
                        and therefore any orthonormal set of vectors generates a unique sequence of operators forming a POVM in this way, and it is clear that this construction inverts the previous mapping from sequences of operators forming a POVM to sets of vectors. While this is not a bijective mapping between POVMs and orthonormal sets of vectors, since any POVM gives rise to \(2^{o} o!\) such sequences (since any of the operators \(\sqrt{M_a}\) could be replaced by \(\sqrt{M_a}^{\dagger}\), and the operators permuted, while giving the same POVM up to relabelling of outcomes), every POVM gives rise to the same number of sequences. This is in agreement with~\cite{RandPOVM}, where a distribution over the isometries \(V\) (that is, the sets of \(d\) orthonormal vectors) is shown to recover the uniform distribution on the Lebesgue measure over the set of POVMs.

                        We parametrise the sets of orthonormal \(d\) vectors \({\{\ket{\gamma^{(g)}}_{\Hs}\}}_{g=1}^{d}\) by parametrising the unitary \(U_{\gamma}\) that generates them from the computational basis \({\{\ket{g}_{\Hs}\}}_{g=1}^{od}\) as \(\ket{\gamma^{(g)}}_{\Hs} = U_{\gamma}\ket{g}_{\Hs}\). From \Cref{eqn:unitaryparam_Theta} and \Cref{eqn:unitaryparam_phase}, it is clear that parameters \(\theta_{j,k}\) with \(j > d\) and \(k > d\) will not appear in the expression for the set of orthonormal vectors. Therefore, we can write
                        \begin{equation}
                          U_{\gamma} = \prod_{j=1}^d \left( \prod_{k=j+1}^{od} \exp(i\theta_{k,j}\ket{k}\bra{k}) \exp(i\theta_{j,k}\sigma_{j,k}) \right) \left[ \prod_{l=1}^d \exp\left(i\theta_{l,l}\ket{l}\bra{l}\right) \right],
                        \end{equation}
                        specifying the unitary with \((2o-1)d^2\) real parameters.

                        Decomposing \(\ket{\gamma^{(g)}}\) as a direct sum, we have
                        \begin{align}
                          g_a\ket{\gamma^{(g)}_a} &= \left(\bra{a}_O \otimes I_A\right)U_{\gamma}\ket{g}_{\Hs} \\
                        ...
                        \begin{appendices}
                          ...
                          \begin{equation}
                            M\left(\sum_i a_i\cvec{v}_i\right) = \sum_i a_i M\cvec{v}_i.
                          \end{equation}
                          ...
                          \end{equation}
                          for some basis \(\{e_i\}\). It is linear: \(\Tr{M+N} = \Tr{M} + \Tr{N}\). Remarkably, this operation is basis-independent: any basis will give the same answer, which is the sum of \(M\)'s eigenvalues, or in a matrix representation, the sum of the diagonal elements. A crucial property of the trace is that it is \emph{invariant under cyclic permutations}: \(\Tr{A_1 A_2 \cdots A_n} = \Tr{A_n A_1 \cdots A_{n-1}}\).
                          ...
                          \end{equation}
                          We typically use this notation to avoid some tedious clarifications when working with tensor products of Hilbert spaces. For example, let \(\rho \coloneqq \rho_{ABC}\) and \(\sigma \coloneqq \rho_{ACB}\). Then we have \(\rho \cong \sigma\), and with bases \({\{\ket{a_i}\}}_i\), \({\{\ket{b_j}\}}_j\), \({\{\ket{c_k}\}}_k\), the isomorphism is \(\rho \mapsto N\rho{N}^{\dagger}\), with
                          \[ N = \sum_{ijk} \bra{a_i \otimes c_k \otimes b_j} \ket{a_i \otimes b_j \otimes c_k}, \]
                          and this can be computed efficiently by reordering the elements of the matrix representing the state.
                          ...
                          \end{align}
                          Like the trace, this is a linear operation, so this definition can be extended by linearity to compute the partial trace on any state. The convention for joint states is to label all their subsystems, so that the state \(\rho_{AB}\) has two subsystems corresponding to the Hilbert spaces \(A\) and \(B\). In that case, we obtain the marginal state on \(A\) by tracing out \(B\): \(\Tr[_B]{\rho_{AB}} = \rho_{A}\); and the marginal state on \(B\) by tracing out \(A\): \(\Tr[_A]{\rho_{AB}} = \rho_{B}\).
                          ...
                          The phenomenon of convexity arises naturally from physical constraints, and is an essential tool for enabling global optimisation. All statements made without citation in this \lcnamecref{sec:prelim_cvxgeom} can be found in Chapters 2 and 3 of~\cite{BoydVand}.
                          ...
                          Convex combinations are particularly interesting to us, because they represent \emph{probabilistic mixtures} of \(\{x_i\}\), with \(\{w_i\}\) specifying the pmf. An \emph{extreme point} is a point that cannot be written as a convex combination of other points. All extreme points are located on the boundary of a convex set, and closed bounded convex set is the convex hull of its extreme points~\cite[Ch. B.4]{LuenbergerYe}. \emph{Caratheodory's theorem on convex hulls} tells us that, for the convex hull \(S\) of a set of points \(V \subseteq \R^{d}\), any point in \(S\) can be expressed as the convex combination of at most \(d+1\) points of \(V\)~\cite{Caratheodory,CaratheodorySteinitz}.
                          ...
                          Convex \emph{functions} are those with a convex domain \(\Omega\subseteq\R^d\) and a convex \emph{epigraph}, that is
                          \begin{equation}
                          ...
                          \end{equation}
                          that is, for all probability distributions on \(\Omega\).\footnote{This applies to discrete distributions as well, but for brevity we give only the integral form.}
                          ...
                          \begin{equation}
                            f\text{ convex} \Leftrightarrow \nabla^2{f}(\cvec{x}) \succeq 0 \;\forall \cvec{x} \in \Omega.
                          \end{equation}
