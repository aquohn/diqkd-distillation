...
\note{
  One might wonder what nonlocality could have to do with security. The following intuitive argument explains the connection. Let's go back to the scenario with Alice and Bob, and let's say they play the coin-flipping game and use the outcomes \((a,b)\) to derive a shared secret key. However, this time, their jars are packed by an adversary, Eve, who is trying to steal their secret key, so that she determines the coins that are used in each round based on a random outcome \(e\), and is aware of the value of \(e\) in each round. Then, their behaviour will look like this.

  Now, if the \(p_e\)'s are deterministic, such that knowing \(x, y\) and \(e\) would determine \(a, b\), then if Eve finds \(x, y\) out, she can use her knowledge of these values to determine \(a, b\), which would compromise the generated keys. Most DIQKD protocols involve announcing the values \(x, y\), so if the behaviour can be expressed as a mixture of deterministic behaviours in this way, then those protocols cannot generate a secret key. The set of behaviours that can be expressed in this way, however, is precisely \(\Ls\).

  On the other hand, quantum behaviours are fundamentally indeterministic. If we are able to certify that some component of the behaviour was indeterministic even for Eve, then the outcomes \(a, b\) have some degree of security against her. This is the intuition behind DIQKD.
}
...
\note{
  As Charles Bennett once joked, DIQKD still has to be DIY-QKD: classical attacks on the classical processing are still possible.
}
  ...
  \begin{itemize}[<+->]
    \item \(\rho_{\crv{A}^n\crv{X}^n \crv{B}^n\crv{Y}^n E^n}\): Alice--Bob--Eve ccq state after \(n\) Bell tests
          ...
          & \prin^n(\cvec{x}^n,\cvec{y}^n) p^n(\cvec{a}^n\cvec{b}^n|\cvec{x}^n\cvec{y}^n) \proj[\crv{A}^n\crv{X}^n \crv{B}^n\crv{Y}^n]{\cvec{a}^n\cvec{x}^n \cvec{b}^n\cvec{y}^n} \\
            \otimes & \rho_{E^n|\cvec{a}^n\cvec{b}^n\cvec{x}^n\cvec{y}^n}
          \end{aligned} \]
...
\begin{frame}{Security from Behaviours}
    \begin{itemize}[<+->]
      ...
      \item Focus on \emph{iid secret key capacity}
        ...
        over asymptotically secure protocols \(\proto{P}\)
    \end{itemize}
\end{frame}
...
% TODO update tikz to label behaviours and add |\lambda to wiring maps
  ...
  \begin{onlyenv}<1-2>
    \begin{itemize}[<+->]
      \item Generalise to multiple boxes
        ...
        \end{align*}
      \item Marginal wirings must obey \emph{no-retrocausation} conditions:
        \begin{align*}
...
\begin{frame}{Couplers}
  \begin{itemize}[<+->]
    ...
    \item \(\#\text{ extremal couplers} < \#\text{ extremal marginal conditional wiring distributions}\)
